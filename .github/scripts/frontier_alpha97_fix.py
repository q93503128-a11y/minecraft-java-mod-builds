...
start = s.index('    private static boolean builderWithinSiteWorkEnvelope(')
end = s.index('    private static boolean hasReachableGroundWorkPosition(', start)
s = s[:start] + '''    private static boolean builderWithinSiteWorkEnvelope(ServerLevel level, ConstructionState construction, BuildingType type,
...
start = s.index('    private static Path createReachablePath(')
end = s.index('    private static boolean moveToReachable(', start)
s = s[:start] + '''    private static Path createReachablePath(FrontierWorkerEntity builder, BlockPos target) {
...
strict = s.find('    private static boolean pathNodesCurrentlyClear(')
if strict >= 0:
    end = s.index('private static boolean blocksCurrentPathCell(', strict)
    s = s[:strict] + '    ' + s[end:]
...
anchor = '    private static List<GradeCell> createGradePlan(ServerLevel level, ConstructionState construction, BuildingType type) {\n'
helper = '''    private static boolean hasReachableGradeWorkPosition(ServerLevel level, FrontierWorkerEntity builder, BlockPos target) {
...
'''
if anchor not in s:
    raise SystemExit('grade plan anchor not found')
...
ctx.write_text(c.replace(old, new, 1), encoding='utf-8')
