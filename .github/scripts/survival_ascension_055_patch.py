...
ROOT = Path(__file__).resolve().parents[2] / "projects" / "survival-ascension"
...
def replace_once(rel: str, old: str, new: str) -> None:
    text = read(rel)
    if text.count(old) != 1:
        raise SystemExit(f"expected exactly one patch anchor in {rel}: {old[:120]!r}; got {text.count(old)}")
...
# Version + pack lock.
replace_once("gradle.properties", "mod_version=0.54.0-alpha.1", "mod_version=0.55.0-alpha.1")
lock_path = ROOT / "modpack/content-lock.json"
lock = json.loads(lock_path.read_text(encoding="utf-8"))
if lock.get("version") != "0.54.0-alpha.1-content-preview.1":
    raise SystemExit(f"unexpected content lock version: {lock.get('version')!r}")
lock["version"] = "0.55.0-alpha.1-content-preview.1"
lock_path.write_text(json.dumps(lock, ensure_ascii=False, indent=2) + "\n", encoding="utf-8")

# Minecraft 26.2 Sulfur Caves -> Deep expedition. Keep sulfur/cinnabar out of valuable_ores.
deep_path = ROOT / "src/main/resources/data/survivalascension/tags/worldgen/biome/expedition/deep.json"
deep = json.loads(deep_path.read_text(encoding="utf-8"))
values = deep.setdefault("values", [])
if "minecraft:sulfur_caves" not in values:
    values.insert(0, "minecraft:sulfur_caves")
deep_path.write_text(json.dumps(deep, ensure_ascii=False, indent=2) + "\n", encoding="utf-8")

# Dedicated spear affix category. Spear intentionally stays OUT of GEAR_CATEGORIES, preserving acquisition.
...
replace_once(affix, '            case WEAPON -> "파급"; case RANGED -> "산개";', '            case WEAPON -> "파급"; case SPEAR -> "돌파"; case RANGED -> "산개";')
replace_once(affix, '            case WEAPON -> "사냥"; case RANGED -> "연쇄";', '            case WEAPON -> "사냥"; case SPEAR -> "대열"; case RANGED -> "연쇄";')
replace_once(affix, '            case WEAPON -> "충격"; case RANGED -> "충격";', '            case WEAPON -> "충격"; case SPEAR -> "충압"; case RANGED -> "충격";')
replace_once(affix, 'WEAPON("weapon"), RANGED("ranged"), PICKAXE("pickaxe")', 'WEAPON("weapon"), SPEAR("spear"), RANGED("ranged"), PICKAXE("pickaxe")')

# Spear physical scale: real forward momentum only, narrow hostile line, zero damage and zero XP.
combat = "src/main/java/kr/moonseungjun/survivalascension/combat/CombatProgression.java"
replace_once(combat, dedent('''\
...
# User-facing equipment flows.
replace_all("src/main/java/kr/moonseungjun/survivalascension/client/EquipmentRadialMenuScreen.java",
    "검/메이스/활/쇠뇌/곡괭이/도끼/삽/괭이/방어구/방패 표준 태그 장비 필요",
    "검/스피어/메이스/활/쇠뇌/곡괭이/도끼/삽/괭이/방어구/방패 표준 태그 장비 필요")
replace_all("src/main/java/kr/moonseungjun/survivalascension/equipment/EquipmentReforgeService.java",
    "검/메이스/활/쇠뇌/곡괭이/도끼/삽/괭이/방어구/방패 태그 장비",
    "검/스피어/메이스/활/쇠뇌/곡괭이/도끼/삽/괭이/방어구/방패 태그 장비")

# Runtime banner/version.
main = "src/main/java/kr/moonseungjun/survivalascension/SurvivalAscension.java"
replace_once(main, 'public static final String VERSION = "0.54.0-alpha.1";', 'public static final String VERSION = "0.55.0-alpha.1";')
replace_once(main,
...
replace_once(main, "loaded: scaled mastery + mace outer impact rings + shield guard waves",
                   "loaded: scaled mastery + spear momentum drive lines + mace outer impact rings + shield guard waves")

# Guide + docs.
replace_once("src/main/java/kr/moonseungjun/survivalascension/client/GuideScreen.java",
    '                h("삽 토공"),',
...
replace_once("README.md", "## 0.54.0-alpha.1 — Mace Impact Ascension / 메이스 충격권 승천", dedent('''\
...
replace_once("PROJECT.md", "- Mod version: `0.54.0-alpha.1`", "- Mod version: `0.55.0-alpha.1`")
replace_once("PROJECT.md", "## 0.54 Mace Impact Ascension / 메이스 충격권 승천", dedent('''\
...
replace_once("CHANGELOG.md", "## 0.54.0-alpha.1", dedent('''\
## 0.55.0-alpha.1
...
- Drive line is hostile-only, zero-damage/zero-XP, Shift-suppressible and knockback-resistance-aware; base reach/targets scale 3.5/1 -> 4.5/2 -> 5.5/3 -> 6.5/4 -> Field Mastery 7.5/5.
...
replace_once("MODPACK_COMPAT_MATRIX.md",
    "현재는 **태그/타입 기반 통합 + 외부 장비 승천 각인 + 표준 활/쇠뇌 원거리 affix/발사체 스냅샷 + 표준 방패 성공차단 방어 파동 + 표준 메이스 실제스매시 외곽 충격권 + BOP 원정 바이옴 브리지 + 데이터 기반 외부 강적 가중치**까지 연결되어 있다.",
    "26.2 네이티브 경계도 같은 원칙으로 연결한다. `minecraft:sulfur_caves`는 심층권 원정에 들어가며 Sulfur/Cinnabar는 pickaxe-mineable 일반 지형으로 숙련을 받되 `valuable_ores` 광맥/추출로 승격하지 않는다. `minecraft:spears`는 전용 승천 각인 카테고리로 들어가고, 바닐라 Jab/Charge는 유지한 채 실제 전진 운동량이 있을 때만 0피해/0XP 돌파선 밀침을 추가한다.\n\n현재는 **태그/타입 기반 통합 + 외부 장비 승천 각인 + 표준 스피어 운동량 돌파선 + 표준 활/쇠뇌 원거리 affix/발사체 스냅샷 + 표준 방패 성공차단 방어 파동 + 표준 메이스 실제스매시 외곽 충격권 + BOP/26.2 원정 바이옴 브리지 + 데이터 기반 외부 강적 가중치**까지 연결되어 있다.")
replace_once("MODPACK_PLAN_DRAFT.md", "## 현재 0.46 장비 통합 상태", dedent('''\
...
# Release source audit: version sync + exact 0.55 regression checks.
src_test = "tools/test_release_source.py"
replace_once(src_test, 'REQUIRED_VERSION = "0.54.0-alpha.1"', 'REQUIRED_VERSION = "0.55.0-alpha.1"')
replace_all(src_test, 'VERSION = \\"0.54.0-alpha.1\\"', 'VERSION = \\"0.55.0-alpha.1\\"')
replace_all(src_test, "검/메이스/활/쇠뇌/곡괭이/도끼/삽/괭이/방어구/방패 태그 장비", "검/스피어/메이스/활/쇠뇌/곡괭이/도끼/삽/괭이/방어구/방패 태그 장비")
replace_all(src_test, "검/메이스/활/쇠뇌/곡괭이/도끼/삽/괭이/방어구/방패 표준 태그 장비 필요", "검/스피어/메이스/활/쇠뇌/곡괭이/도끼/삽/괭이/방어구/방패 표준 태그 장비 필요")
replace_once(src_test, "Mod version: `0.54.0-alpha.1`", "Mod version: `0.55.0-alpha.1`")
...
# Baseline source originally expects only vanilla tool categories; translate to the newest full imprint wording.
text = read(src_test)
text = text.replace(
...
write(src_test, text)
replace_once(src_test, 'print("- README / PROJECT / CHANGELOG / in-game guide are committed and synchronized to 0.54")',
...
replace_once(pack_test, 'REQUIRED_LOCK_VERSION = "0.54.0-alpha.1-content-preview.1"', 'REQUIRED_LOCK_VERSION = "0.55.0-alpha.1-content-preview.1"')
replace_once(pack_test, "baseline.replace('Mod version: `0.48.0-alpha.1`', 'Mod version: `0.54.0-alpha.1`')", "baseline.replace('Mod version: `0.48.0-alpha.1`', 'Mod version: `0.55.0-alpha.1`')")
replace_all(pack_test, "검/메이스/활/쇠뇌/곡괭이/도끼/삽/괭이/방어구/방패 표준 태그 장비 필요", "검/스피어/메이스/활/쇠뇌/곡괭이/도끼/삽/괭이/방어구/방패 표준 태그 장비 필요")
...
replace_once(pack_test, 'print("mace_affix_outer_impact_bridge=PASS")', 'print("mace_affix_outer_impact_bridge=PASS")\nprint("sulfur_caves_deep_bridge=PASS")\nprint("spear_affix_drive_bridge=PASS")')

# Packaged JAR checks for the new paths.
verify = "tools/verify_release_jar.py"
replace_once(verify, '\n\nprint("frontline_freight_manifest_runtime=present")', dedent('''\
...
replace_once(verify, 'print("mace_outer_impact_release_verify=PASS")', 'print("mace_outer_impact_release_verify=PASS")\nprint("sulfur_caves_deep_runtime=present")\nprint("spear_drive_line_runtime=present")\nprint("native_26_2_release_verify=PASS")')

jar_verify = "tools/verify_jar.py"
replace_once(jar_verify, dedent('''\
...
print("Survival Ascension 0.55 patch payload applied")
