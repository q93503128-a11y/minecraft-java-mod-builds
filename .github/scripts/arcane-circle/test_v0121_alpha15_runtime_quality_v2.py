...
require("src/main/java/kr/moonseungjun/arcanecircle/magic/MageGearService.java",
        "DamageTypes.FALL","event.setCanceled(true)","ignoreThreshold","grantStableDescent",
        "STABLE_DESCENT_UNTIL","SLOW_FALLING")
...
require("src/main/java/kr/moonseungjun/arcanecircle/magic/ExpandedSpellEffects.java",
...
script=Path("..")/".."/".github"/"scripts"/"arcane-circle"/"apply_v0121_alpha15_runtime_quality_v2.py"
subprocess.run([sys.executable,str(script)],cwd=ROOT,check=True)
after={rel:digest(rel) for rel in tracked}
changed=[rel for rel in tracked if before[rel]!=after[rel]]
if changed:raise SystemExit(f"alpha.15 v2 migration is not idempotent: {changed}")
print("Arcane Circle alpha.15 runtime-quality audit v2: PASS")
