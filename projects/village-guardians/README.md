...
- 공용 전술 포인트는 별도 재화를 추가하지 않고 기존 트리를 사용한다. 모든 레벨업마다 1P를 얻고, 6~10단계 비용은 5/8/12/18/25P로 증가한다. 50개 전체 비용은 375P라 Lv.300의 299P로 전부 찍을 수 없으며, 기존 세이브에서 이미 소비한 포인트는 회수하지 않는다.
