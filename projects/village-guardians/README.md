...
- 공용 전술 포인트는 별도 재화를 추가하지 않고 기존 트리를 사용한다. 모든 레벨업마다 1P를 얻고, 기존 단계 비용 규칙을 유지해 50개 전체 비용은 110P다. Lv.300 전에 다섯 갈래를 모두 완성할 수 있으며, 한 갈래 10단계 완성 뒤에는 남는 포인트를 최대 체력 +1 / 기본 공격력 +1 반복 단련에 사용할 수 있다. 기존 세이브 투자는 회수하지 않는다.
