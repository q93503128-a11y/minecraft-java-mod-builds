...
def main():
    ...
    assert "double range = (50.0 + 52.0 * mastery) * endlessUtilityAdaptation(rank);" in merc
    assert "double radius = (8.0 + 13.0 * masteryProgress(rank)) * utility;" in merc
    assert "fullMercenaryMastery(rank)" in merc and "endlessDefenseUtilityMultiplier" in merc
