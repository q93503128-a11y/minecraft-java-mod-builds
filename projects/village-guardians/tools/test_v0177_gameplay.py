...
def main():
    assert "mod_version=0.17.7-alpha.1" in (ROOT / "gradle.properties").read_text()
    ...
    assert "Math.max(0, level - 1)" in common and common.count('("mobility_') == 8
    ...
    assert "test_cast:" in ui and "useTestSkill" in role and "targetsNear" in test
