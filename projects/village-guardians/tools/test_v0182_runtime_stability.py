...
base_tornado = ability.split("case ARCANIST_CHAIN -> {", 1)[1].split("case ARCANIST_NOVA -> {", 1)[0]
require("specialRank, 0, forward" in base_tornado
        and '"arcanist_tornado".equals(kind())' not in effect
        and "area.moveTo(area.center().add(travel.scale(0.24)))" in ability,
        "Tornado visual and gameplay share the fixed cast direction and same 0.24-block-per-tick speed")
...
require("Math.min(7.0f," not in enemy
        and "VillageCampaignProgression.effectiveCombatDay(VillageCouncilState.currentDay()) * 0.135f" in enemy,
        "Long-campaign unavoidable magic damage uses an uncapped soft-growth effective-day bonus")
