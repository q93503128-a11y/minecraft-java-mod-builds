...
- Existing-world compatibility: `village_council` 저장 키를 유지하고 마을 중심과 RPG 필드는 선택값으로 추가한다.
...
7. RPG 성장은 실제 공격, 생존력, 추가 체력과 역할 스킬에 반영한다.
8. 마을에는 영구 저장되는 중심 좌표가 있고, 오버월드 반경 64블록을 공식 마을 영역으로 사용한다.
9. 마을 영역에서 벌어지는 방어 전투는 일반 사냥보다 높은 성장 보상을 제공한다.
10. UI는 기능을 가리는 임시 회색 화면으로 만들지 않는다. 제대로 디자인할 시점 전에는 명령 기반으로 유지한다.
11. 최종 배포물은 소스 ZIP이 아닌 실제 컴파일된 실행용 JAR이다.
...
30레벨 기준 수치는 기본 공격 약 6.23배, 받는 피해 28%, 추가 체력 36포인트다. 이후 습격 난도와 보상도 이 성장 폭과 마을 영역을 기준으로 함께 강화한다.
