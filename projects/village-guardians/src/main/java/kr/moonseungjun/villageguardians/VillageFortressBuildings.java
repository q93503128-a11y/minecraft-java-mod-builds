...
final class VillageFortressBuildings {
    ...
    static BlockPos attackPoint(
        ...
        if (building == VillageProgressionSystem.Building.WALLS) {
            ...
            return new BlockPos(x, villageCenter.getY(),
                    villageCenter.getZ() - VillageWorldSystem.FORTRESS_RADIUS + 5);
        }
        ...
        if (enemy.getX() < bounds.x0()) {
            targetX = bounds.x0() - 1;
        } else if (enemy.getX() > bounds.x1()) {
            targetX = bounds.x1() + 1;
        }
        if (enemy.getZ() < bounds.z0()) {
            targetZ = bounds.z0() - 1;
        } else if (enemy.getZ() > bounds.z1()) {
            targetZ = bounds.z1() + 1;
        }
    ...
    static boolean isTouchingStructure(
            BlockPos villageCenter,
            VillageProgressionSystem.Building building,
            BlockPos enemy) {
        if (building == VillageProgressionSystem.Building.WALLS) {
        ...
        Bounds bounds = bounds(villageCenter, building);
        return enemy.getX() >= bounds.x0() - 2
                && enemy.getX() <= bounds.x1() + 2
                && enemy.getZ() >= bounds.z0() - 2
                && enemy.getZ() <= bounds.z1() + 2
                && enemy.getY() >= villageCenter.getY() - 2
                && enemy.getY() <= villageCenter.getY() + bounds.height() + 3;
    }

    static long distanceSquaredToStructure(
            BlockPos villageCenter,
            VillageProgressionSystem.Building building,
            BlockPos pos) {
        if (building == VillageProgressionSystem.Building.WALLS) {
    ...
    static void applyUpgradeVisual(
            ...
            VillageProgressionSystem.Building building,
            int upgradeLevel) {
        // Upgrades are represented by defence power and management status, not loose block piles.
    }
