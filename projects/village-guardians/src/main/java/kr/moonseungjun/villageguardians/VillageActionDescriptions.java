...
final class VillageActionDescriptions {
    ...
    static String describe(String action, String label) {
        ...
        if (action.startsWith("manage:")) return label + "의 내구도와 다음 강화 효과를 관리합니다.";
        if (action.startsWith("repair:")) return label + "\n공동 보급품을 사용해 시설 내구도를 최대치로 복구합니다.";
        if (action.startsWith("upgrade:")) return label + "\n공동 보급품을 사용해 시설 등급과 고유 효과를 강화합니다.";
        if (action.startsWith("select_role:")) return label + "\n회관에서 현재 직업을 변경합니다.";
        if (action.startsWith("skill_node:")) return label + "\n전술 포인트 1개를 사용합니다.";
        if (action.startsWith("role_node:")) return label + "\n요구 레벨과 수호 주화를 사용해 직업 공통 효과를 습득합니다.";
        if (action.startsWith("role_skill_unlock:")) return label + "\n요구 레벨과 수호 주화를 사용해 장착 기술을 습득합니다.";
        if (action.startsWith("gear:")) return label + "\n표시된 레벨·방어 일수 조건과 수호 주화를 사용해 성장 장비를 구매합니다.";
        return switch (action) {
            case "open_status" -> "수호자 상태와 인벤토리 동작을 확인합니다.";
            case "open_skill_tree" -> "드래그·확대·축소 가능한 공용 전술 발전 화면을 엽니다.";
            case "open_equipment_shop" -> "레벨과 방어 일수에 따라 해금되는 장비 상점을 엽니다.";
            ...
            case "train" -> label + "\n병영 훈련으로 XP를 획득합니다.";
            case "hire_mercenary" -> "철 주괴 24개를 사용해 사망 전까지 지속되는 마을 용병을 고용합니다.";
            case "defense_status" -> "노포·화염·빙결·비전 방어탑과 배치된 용병 수를 확인합니다.";
            ...
            default -> label == null || label.isBlank() ? "선택한 동작을 실행합니다." : label + "을(를) 실행합니다.";
        };
    }

    static boolean requiresConfirmation(String action) {
        if (action == null) return false;
        return action.startsWith("repair:") || action.startsWith("upgrade:")
                || action.startsWith("select_role:") || action.startsWith("skill_node:")
                || action.startsWith("role_node:") || action.startsWith("role_skill_unlock:")
                || action.startsWith("gear:") || action.equals("buy_arrows") || action.equals("buy_food")
                || action.equals("sell_loot") || action.equals("forge_upgrade") || action.equals("skill_learn")
                || action.equals("train") || action.equals("hire_mercenary")
                || action.equals("return_village") || action.startsWith("restart_");
    }

    static String executeLabel(String action) {
        if (action == null) return "선택 필요";
        if (action.startsWith("select_role:")) return "직업 배치";
        if (action.startsWith("skill_node:") || action.startsWith("role_node:")) return "효과 습득";
        if (action.startsWith("role_skill_unlock:")) return "기술 습득";
        if (action.startsWith("gear:") || action.startsWith("buy_")) return "구매";
