...
import dev.moonseungjun.fishinggame.progression.FishingRebirthService;
...
public final class FishingNetworking {
    ...
    public static void initialize() {
        PayloadTypeRegistry.serverboundPlay().register(CastStartPayload.TYPE, CastStartPayload.CODEC);
        ...
        PayloadTypeRegistry.serverboundPlay().register(RebirthPayload.TYPE, RebirthPayload.CODEC);
        ...
        ServerPlayNetworking.registerGlobalReceiver(
                ...
                (payload, context) -> FishingSessionManager.startCastCharge(context.player(), payload.offHand())
        );
        ServerPlayNetworking.registerGlobalReceiver(
        ...
        ServerPlayNetworking.registerGlobalReceiver(
                RebirthPayload.TYPE,
                (payload, context) -> {
                    if (payload.requested()) FishingRebirthService.request(context.player());
                }
        );
        ServerPlayNetworking.registerGlobalReceiver(
