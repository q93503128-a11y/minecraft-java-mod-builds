...
with zipfile.ZipFile(jar) as archive:
    ...
    if version!='0.12.1-alpha.64': raise SystemExit(f'unexpected alpha.64 package version: {version}')
    ...
    if city.get('baseline_radius')!=112 or city.get('baseline_strikes')!=49 or city.get('range_scaled') is not True: raise SystemExit('alpha.64 Meteor Cityfall magnitude metadata mismatch')
    if set(city.get('projectile_scaling',[]))!={'count','spacing','body_scale','fall_height'}: raise SystemExit('alpha.64 projectile scaling metadata mismatch')
    if city.get('terminal_shock')!='full_cityfall_radius_with_inner_lethal_core': raise SystemExit('alpha.64 terminal shock metadata mismatch')
    if city.get('terrain')!='budgeted_citywide_crater_lattice': raise SystemExit('alpha.64 terrain lattice metadata mismatch')
    if index.get('ninth_circle_divine_scale_phase')!='meteor_cityfall_foundation': raise SystemExit('alpha.64 divine scale phase metadata missing')
    if index.get('crown_meteor')!='range_scaled_cityfall_barrage_then_delayed_crown_cataclysm': raise SystemExit('alpha.64 Crown Meteor metadata missing')
    if 'range_scaled_cityfall_meteor_swarm' not in set(index.get('ninth_circle_deep_audit',[])): raise SystemExit('alpha.64 ninth-circle Meteor audit metadata missing')
    if index.get('grimoire_ui')!='primary_book_style_restored_for_atlas_and_academy': raise SystemExit('alpha.63 Primary Grimoire restore regressed')
...
print('Arcane Circle alpha.64 JAR verification: PASS')
print('alpha64_meteor_cityfall=PASS')
print('alpha64_range_scaled_projectiles=PASS')
print('alpha64_dense_grand_arrays=PASS')
print('alpha63_primary_grimoire_preserved=PASS')
