...
/**
 * Optimized capital builder. It writes only changed blocks and uses update flag 2,
 * avoiding the prototype builder's repeated air writes and neighbour cascades.
 */
public final class TerrainIntegratedCapitalBuilder {
    private TerrainIntegratedCapitalBuilder() {
    }
    ...
    private static void erden(ServerLevel level, RealmSiteLayoutSavedData.RealmSite site) {
        ...
        Style style = new Style(Blocks.STONE_BRICKS, Blocks.STRIPPED_SPRUCE_LOG,
                Blocks.BIRCH_PLANKS, Blocks.DARK_OAK_PLANKS, Blocks.SPRUCE_PLANKS);

        grade(level, cx - 124, cz - 104, cx + 124, cz + 104, y, Blocks.GRASS_BLOCK, Blocks.DIRT);
        blendEdge(level, cx - 124, cz - 104, cx + 124, cz + 104, y, Blocks.GRASS_BLOCK);
        fortification(level, cx, y, cz, 120, 100, style);
        road(level, cx, cz - 100, cx, cz + 100, y, 4, Blocks.PACKED_MUD, Blocks.STONE_BRICKS);
        road(level, cx - 120, cz, cx + 120, cz, y, 4, Blocks.PACKED_MUD, Blocks.STONE_BRICKS);
        ...
        keep(level, cx - 34, y, cz - 91, style);
        civicHall(level, cx + 18, y, cz - 82, 28, 18, style, "행정청");
        temple(level, cx - 74, y, cz - 78);
        market(level, cx, y, cz);
        civicHall(level, cx + 47, y, cz + 14, 21, 14, style, "여관");
        civicHall(level, cx - 77, y, cz + 17, 23, 15, style, "길드");
        smithy(level, cx + 78, y, cz - 35, style);
        barracks(level, cx - 106, y, cz - 39, style);
        civicHall(level, cx + 82, y, cz + 72, 19, 12, style, "곡물창고");
        stable(level, cx - 98, y, cz + 71);
        ...
        for (int i = 0; i < homes.length; i++) {
            building(level, cx + homes[i][0], y, cz + homes[i][1], 10 + i % 3,
                    8 + (i + 1) % 3, 5, style);
        }

        canal(level, cx - 164, y, cz - 142, cz + 142);
        stoneBridge(level, cx - 172, y + 1, cz, cx - 118, y + 1, cz);
        road(level, cx - 164, cz, cx - 120, cz, y + 1, 3, Blocks.GRAVEL, Blocks.COBBLESTONE);
        dock(level, cx - 166, y, cz + 67);
        farm(level, cx + 146, y, cz + 57, 34, 27);
        farm(level, cx + 146, y, cz - 18, 34, 27);
        farm(level, cx - 196, y, cz + 78, 32, 25);
        orchard(level, cx + 150, y, cz - 88, 6, 5, Blocks.OAK_LOG, Blocks.OAK_LEAVES);
        outerTrees(level, cx, cz, 146, 28, Blocks.OAK_LOG, Blocks.OAK_LEAVES);
    }
    ...
    private static void silvana(ServerLevel level, RealmSiteLayoutSavedData.RealmSite site) {
        ...
        elvenLodge(level, cx + 56, y + 1, cz - 21, 10);
        elvenLodge(level, cx - 39, y + 1, cz + 55, 10);
        elvenLodge(level, cx + 46, y + 2, cz + 58, 12);
        platform(level, cx - 58, y + 16, cz - 30, 10);
        platform(level, cx + 56, y + 15, cz - 21, 9);
        canopyBridge(level, cx, y + 23, cz, cx - 58, y + 16, cz - 30);
        canopyBridge(level, cx, y + 23, cz, cx + 56, y + 15, cz - 21);
        moonGarden(level, cx + 86, y, cz + 80);
        councilCircle(level, cx - 82, y, cz + 72);
        road(level, cx, cz, cx + 86, cz + 80, y, 2, Blocks.ROOTED_DIRT, Blocks.MOSS_BLOCK);
        road(level, cx, cz, cx - 82, cz + 72, y, 2, Blocks.ROOTED_DIRT, Blocks.MOSS_BLOCK);
        road(level, cx, cz, cx, cz - 105, y, 2, Blocks.ROOTED_DIRT, Blocks.MOSS_BLOCK);
        outerTrees(level, cx, cz, 108, 64, Blocks.DARK_OAK_LOG, Blocks.FLOWERING_AZALEA_LEAVES);
    }
    ...
    private static void kardum(ServerLevel level, RealmSiteLayoutSavedData.RealmSite site) {
        ...
        Style style = new Style(Blocks.DEEPSLATE_BRICKS, Blocks.POLISHED_BASALT,
                Blocks.POLISHED_ANDESITE, Blocks.DEEPSLATE_TILES, Blocks.POLISHED_DEEPSLATE);
        grade(level, cx - 108, cz - 98, cx + 108, cz + 102, y, Blocks.STONE, Blocks.STONE);
        terraces(level, cx, y, cz);
        dwarvenGate(level, cx, y, cz - 88);
        civicHall(level, cx - 38, y + 6, cz - 34, 27, 18, style, "연맹회관");
        civicHall(level, cx + 19, y + 9, cz - 20, 29, 19, style, "석조대회랑");
        forge(level, cx - 16, y + 3, cz + 42, style);
        quarter(level, cx - 82, y, cz + 34, style);
        quarter(level, cx + 64, y + 3, cz + 38, style);
        vault(level, cx + 58, y + 10, cz - 70, style);
        stoneRoad(level, cx, y, cz - 88, cx, y + 6, cz + 80);
        stoneRoad(level, cx - 94, y, cz + 34, cx + 94, y + 3, cz + 34);
        mine(level, cx - 105, y + 2, cz - 58);
        oreYard(level, cx + 94, y + 1, cz + 72);
    }

    private static void keep(ServerLevel level, int x, int y, int z, Style s) {
        building(level, x, y, z, 33, 24, 9, s);
        tower(level, x - 4, y, z - 4, 8, 15, s);
        tower(level, x + 29, y, z - 4, 8, 15, s);
        tower(level, x - 4, y, z + 20, 8, 15, s);
        tower(level, x + 29, y, z + 20, 8, 15, s);
    }

    private static void civicHall(ServerLevel level, int x, int y, int z, int w, int d, Style s, String ignoredRole) {
        building(level, x, y, z, w, d, 7, s);
    }

    private static void building(ServerLevel level, int x, int y, int z, int w, int d, int h, Style s) {
        prepareLot(level, x - 3, z - 3, x + w + 2, z + d + 2, y, s.foundation());
        fill(level, x, y, z, x + w - 1, y, z + d - 1, s.floor());
        for (int dx = 0; dx < w; dx++) for (int dz = 0; dz < d; dz++) {
            if (dx != 0 && dx != w - 1 && dz != 0 && dz != d - 1) continue;
            for (int dy = 1; dy <= h; dy++) {
                boolean beam = ((dx == 0 || dx == w - 1) && (dz == 0 || dz == d - 1))
                        || dy == 1 || dy == h || dx % 5 == 0;
                set(level, x + dx, y + dy, z + dz, beam ? s.frame() : s.wall());
            }
        }
        clear(level, x + w / 2 - 1, y + 1, z, x + w / 2 + 1, y + 4, z);
        roof(level, x - 2, y + h + 1, z - 2, w + 4, d + 4, s.roof());
        set(level, x + w / 2, y + 3, z + d - 3, Blocks.LANTERN);
    }

    private static void roof(ServerLevel level, int x, int y, int z, int w, int d, Block roof) {
        int layers = Math.max(3, Math.min(6, d / 2));
        for (int layer = 0; layer < layers; layer++) {
            fill(level, x, y + layer, z + layer, x + w - 1, y + layer, z + layer, roof);
            fill(level, x, y + layer, z + d - 1 - layer, x + w - 1, y + layer, z + d - 1 - layer, roof);
        }
        if (z + layers <= z + d - 1 - layers) {
            fill(level, x, y + layers, z + layers, x + w - 1, y + layers, z + d - 1 - layers, roof);
        }
    }

    private static void temple(ServerLevel level, int x, int y, int z) {
        Style s = new Style(Blocks.SMOOTH_STONE, Blocks.CHISELED_STONE_BRICKS,
                Blocks.CALCITE, Blocks.STONE_BRICKS, Blocks.SMOOTH_STONE);
        building(level, x, y, z, 22, 17, 7, s);
        set(level, x + 11, y + 2, z + 12, Blocks.BELL);
    }

    private static void market(ServerLevel level, int cx, int y, int cz) {
        prepareLot(level, cx - 25, cz - 21, cx + 25, cz + 21, y, Blocks.STONE_BRICKS);
        for (int[] o : new int[][]{{-20, -14}, {-7, -14}, {7, -14}, {-20, 9}, {-7, 9}, {7, 9}}) {
            int x = cx + o[0];
            int z = cz + o[1];
    ...
    private static void smithy(ServerLevel level, int x, int y, int z, Style s) {
        building(level, x, y, z, 16, 11, 6, s);
    ...
    private static void barracks(ServerLevel level, int x, int y, int z, Style s) {
        building(level, x, y, z, 29, 18, 7, s);
    ...
    private static void fortification(ServerLevel level, int cx, int y, int cz, int rx, int rz, Style s) {
        for (int x = cx - rx; x <= cx + rx; x++) {
            wallColumn(level, x, y, cz - rz, s.foundation());
            wallColumn(level, x, y, cz + rz, s.foundation());
        }
        for (int z = cz - rz; z <= cz + rz; z++) {
            wallColumn(level, cx - rx, y, z, s.foundation());
            wallColumn(level, cx + rx, y, z, s.foundation());
        }
        for (int[] c : new int[][]{{-rx, -rz}, {rx, -rz}, {-rx, rz}, {rx, rz}}) {
            tower(level, cx + c[0] - 4, y, cz + c[1] - 4, 9, 13, s);
        }
        gate(level, cx, y, cz - rz, true, s);
        gate(level, cx, y, cz + rz, true, s);
        gate(level, cx - rx, y, cz, false, s);
        gate(level, cx + rx, y, cz, false, s);
    }
    ...
    private static void gate(ServerLevel level, int x, int y, int z, boolean northSouth, Style s) {
        ...
        for (int px = x1; px <= x2; px++) for (int pz = z1; pz <= z2; pz++) {
            ...
            for (int dy = 1; dy <= 9; dy++) set(level, px, y + dy, pz, s.foundation());
        }
        if (northSouth) clear(level, x - 4, y + 1, z1, x + 4, y + 6, z2);
        else clear(level, x1, y + 1, z - 4, x2, y + 6, z + 4);
        fill(level, x1 - 1, y + 10, z1 - 1, x2 + 1, y + 10, z2 + 1, s.roof());
        set(level, x, y + 9, z, Blocks.BELL);
    }

    private static void tower(ServerLevel level, int x, int y, int z, int size, int height, Style s) {
        prepareLot(level, x - 1, z - 1, x + size, z + size, y, s.foundation());
        for (int dx = 0; dx < size; dx++) for (int dz = 0; dz < size; dz++) {
            if (dx != 0 && dx != size - 1 && dz != 0 && dz != size - 1) continue;
            for (int dy = 1; dy <= height; dy++) set(level, x + dx, y + dy, z + dz, s.foundation());
        }
        fill(level, x - 1, y + height + 1, z - 1, x + size, y + height + 1, z + size, s.roof());
    ...
    private static void stoneBridge(ServerLevel level, int x1, int y1, int z1, int x2, int y2, int z2) {
    ...
    private static void outerTrees(ServerLevel level, int cx, int cz, int radius, int count, Block log, Block leaves) {
        for (int i = 0; i < count; i++) {
            ...
            tree(level, x, RealmSitePlanner.surfaceY(level, x, z), z, log, leaves, 5 + i % 4);
        }
    }
    ...
    private static void forge(ServerLevel level, int x, int y, int z, Style s) {
        building(level, x, y, z, 30, 20, 8, s);
    ...
    private static void quarter(ServerLevel level, int x, int y, int z, Style s) {
        for (int i = 0; i < 4; i++) building(level, x + (i % 2) * 22, y + (i / 2) * 2,
                z + (i / 2) * 18, 18, 14, 7, s);
    }

    private static void vault(ServerLevel level, int x, int y, int z, Style s) {
        building(level, x, y, z, 24, 18, 8, s);
    ...
    private static void road(ServerLevel level, int x1, int z1, int x2, int z2, int y, int halfWidth,
        ...
        for (int i = 0; i <= steps; i++) {
            ...
            for (int side = -halfWidth; side <= halfWidth; side++) {
                ...
                prepareColumn(level, px, pz, y, Math.abs(side) == halfWidth ? edge : center, Blocks.DIRT);
            }
        }
    }

    private static void grade(ServerLevel level, int x1, int z1, int x2, int z2, int y, Block surface, Block filler) {
        for (int x = Math.min(x1, x2); x <= Math.max(x1, x2); x++) {
            for (int z = Math.min(z1, z2); z <= Math.max(z1, z2); z++) prepareColumn(level, x, z, y, surface, filler);
        }
    }

    private static void gradeEllipse(ServerLevel level, int cx, int cz, int rx, int rz, int y, Block surface, Block filler) {
        for (int x = cx - rx; x <= cx + rx; x++) for (int z = cz - rz; z <= cz + rz; z++) {
            double nx = (x - cx) / (double) rx;
            double nz = (z - cz) / (double) rz;
            if (nx * nx + nz * nz <= 1.0) prepareColumn(level, x, z, y, surface, filler);
        }
    }

    private static void blendEdge(ServerLevel level, int x1, int z1, int x2, int z2, int y, Block surface) {
        for (int ring = 1; ring <= 12; ring++) {
            int target = y - Math.min(6, (ring + 1) / 2);
            for (int x = x1 - ring; x <= x2 + ring; x++) {
                prepareColumn(level, x, z1 - ring, target, surface, Blocks.DIRT);
                prepareColumn(level, x, z2 + ring, target, surface, Blocks.DIRT);
            }
            for (int z = z1 - ring; z <= z2 + ring; z++) {
                prepareColumn(level, x1 - ring, z, target, surface, Blocks.DIRT);
                prepareColumn(level, x2 + ring, z, target, surface, Blocks.DIRT);
            }
        }
    }

    private static void prepareLot(ServerLevel level, int x1, int z1, int x2, int z2, int y, Block surface) {
        grade(level, x1, z1, x2, z2, y, surface, Blocks.DIRT);
    ...
    private static void prepareColumn(ServerLevel level, int x, int z, int targetY, Block surface, Block filler) {
    ...
    private static void tree(ServerLevel level, int x, int y, int z, Block log, Block leaves, int height) {
        prepareColumn(level, x, z, y, Blocks.GRASS_BLOCK, Blocks.DIRT);
