...
public final class BattleVfx {
    ...
    public static void skill(ServerLevel level, String combatantId, String skillId, Vec3 source, Vec3 target, boolean damaging) {
        ...
        switch (combatantId) {
            ...
            case "P05" -> lynette(level, source, aim, skillId);
    ...
    public static void warning(ServerLevel level, String combatantId, Vec3 center) {
        ParticleOptions primary = switch (combatantId) {
            case "E003", "B04" -> ParticleTypes.FLAME; case "B01" -> ParticleTypes.CLOUD;
            case "B05" -> ParticleTypes.PORTAL; default -> ParticleTypes.CRIT;
        };
    ...
    public static void phase(ServerLevel level, String combatantId, Vec3 center, int phase) {
        ParticleOptions primary = switch (combatantId) {case "B01" -> ParticleTypes.CLOUD;case "B02" -> ParticleTypes.ENCHANT;case "B03" -> ParticleTypes.ELECTRIC_SPARK;case "B04" -> ParticleTypes.FLAME;case "B05" -> ParticleTypes.PORTAL;default -> ParticleTypes.END_ROD;};
        double radius=phase>=3?1.9:1.45;int count=phase>=3?36:26;ring(level,primary,center.add(0,.2,0),radius,count);ring(level,ParticleTypes.END_ROD,center.add(0,1.15,0),radius*.65,count-8);burst(level,primary,center.add(0,1.15,0),phase>=3?30:20,.9,1,.9,.12);
    }
    ...
    public static void down(ServerLevel level, Vec3 center){burst(level,ParticleTypes.SMOKE,center.add(0,.9,0),14,.45,.55,.45,.02);}
    public static void victory(ServerLevel level,Vec3 center){ring(level,ParticleTypes.END_ROD,center.add(0,.3,0),1,18);burst(level,ParticleTypes.ENCHANT,center.add(0,1.4,0),22,.8,.9,.8,.15);}

    private static void kyren(ServerLevel level,Vec3 source,Vec3 target,String skillId){line(level,ParticleTypes.CRIT,source.add(0,1.15,0),target.add(0,1,0),10);if("p01_breaker_strike".equals(skillId)){slashArc(level,ParticleTypes.END_ROD,target.add(0,1,0),1.15,18);burst(level,ParticleTypes.CRIT,target.add(0,1,0),16,.55,.55,.55,.16);}else slashArc(level,ParticleTypes.CRIT,target.add(0,1,0),.8,12);}
    private static void lumea(ServerLevel level,Vec3 source,Vec3 target){ring(level,ParticleTypes.END_ROD,source.add(0,1,0),.85,18);ring(level,ParticleTypes.PORTAL,target.add(0,.9,0),.75,14);line(level,ParticleTypes.END_ROD,source.add(0,1.15,0),target.add(0,1.15,0),7);}
    private static void bram(ServerLevel level,Vec3 source,Vec3 target,boolean damaging){ring(level,ParticleTypes.CLOUD,source.add(0,.2,0),.85,16);burst(level,damaging?ParticleTypes.CRIT:ParticleTypes.END_ROD,(damaging?target:source).add(0,.9,0),14,.6,.45,.6,.08);}

    /** Character wiki: no heart particles; thin ivory/light strands wrap and converge on the target. */
    private static void elysia(ServerLevel level,Vec3 source,Vec3 target,String skillId){
        ...
        ring(level,ParticleTypes.END_ROD,target.add(0,.28,0),.72,16);ring(level,ParticleTypes.ENCHANT,target.add(0,.78,0),.54,14);
        burst(level,ParticleTypes.END_ROD,target.add(0,1.0,0),"p04_returned_breath".equals(skillId)?16:9,.34,.58,.34,.02);
        if("p04_returned_breath".equals(skillId)){ring(level,ParticleTypes.ENCHANT,target.add(0,1.15,0),.92,22);ring(level,ParticleTypes.END_ROD,target.add(0,1.55,0),.48,12);}
    }

    private static void lynette(ServerLevel level,Vec3 source,Vec3 target,String skillId){boolean pierce="p05_piercing_shot".equals(skillId);line(level,pierce?ParticleTypes.END_ROD:ParticleTypes.CRIT,source.add(0,1.35,0),target.add(0,1.05,0),pierce?24:18);burst(level,ParticleTypes.END_ROD,target.add(0,1.05,0),pierce?12:8,pierce?.38:.3,.3,pierce?.38:.3,.06);}
    private static void morwen(ServerLevel level,Vec3 source,Vec3 target){ring(level,ParticleTypes.ENCHANT,source.add(0,1,0),.72,18);line(level,ParticleTypes.ENCHANT,source.add(0,1.15,0),target.add(0,1.1,0),9);burst(level,ParticleTypes.END_ROD,target.add(0,1,0),9,.45,.65,.45,.035);}
    private static void marion(ServerLevel level,Vec3 source,Vec3 target,String skillId){ring(level,ParticleTypes.ENCHANT,source.add(0,.75,0),.82,18);if("p07_summon_toto".equals(skillId)){ring(level,ParticleTypes.CRIT,source.add(0,.12,0),1.15,28);burst(level,ParticleTypes.END_ROD,source.add(0,.7,0),16,.7,.5,.7,.08);}else line(level,ParticleTypes.END_ROD,source.add(0,1,0),target.add(0,1,0),8);}
    private static void toto(ServerLevel level,Vec3 source,Vec3 target){line(level,ParticleTypes.END_ROD,source.add(0,.65,0),target.add(0,.8,0),9);burst(level,ParticleTypes.CRIT,target.add(0,.8,0),8,.35,.3,.35,.08);}
    private static void raze(ServerLevel level,Vec3 source,Vec3 target){burst(level,ParticleTypes.FLAME,source.add(0,1,0),7,.35,.5,.35,.02);line(level,ParticleTypes.CRIT,source.add(0,1.25,0),target.add(0,1,0),11);slashArc(level,ParticleTypes.FLAME,target.add(0,1,0),1,16);}
