...
public final class SouthgateChapterWorld {
    ...
    public record BuiltChapter(
            Vec3 spawn, Vec3 scout,
            Vec3 m01Home, Vec3 m01End,
            Vec3 m02Home, Vec3 m02End,
    ...
    public static boolean contains(BuiltChapter chapter, Vec3 pos) {
        ...
        List<Node> nodes = route();
        for (int i = 0; i < nodes.size() - 1; i++) {
            if (distanceSqToSegment(pos.x, pos.z, nodes.get(i), nodes.get(i + 1)) <= 24.0 * 24.0) return true;
        }
        return near(pos, 0, 132, 24) || near(pos, -12, 153, 22) || near(pos, 13, 176, 22)
                || near(pos, 92, 207, 24) || near(pos, 220, 230, 25)
                || near(pos, 286, 240, 25) || near(pos, 355, 245, 30);
    }
    ...
    private static BuiltChapter built() {
        ...
        return new BuiltChapter(
                new Vec3(0.5, 66.0, 123.5), new Vec3(-6.5, 66.0, 132.5),
                new Vec3(-12.0, 66.0, 151.0), new Vec3(-2.0, 66.0, 157.0),
                new Vec3(13.0, 67.0, 173.0), new Vec3(2.0, 67.0, 181.0),
    ...
    private static void buildSouthGateApproach(ServerLevel level) {
        for (int z = 120; z <= 139; z++) {
            for (int x = -8; x <= 8; x++) {
                if (Math.abs(x) <= 3) set(level, x, 65, z, ((x + z) & 1) == 0 ? Blocks.POLISHED_ANDESITE : Blocks.STONE_BRICKS);
                else if (Math.abs(x) == 8) set(level, x, 66, z, Blocks.STONE_BRICK_WALL);
            }
        }
        lanternPost(level, -7, 65, 126);
        lanternPost(level, 7, 65, 126);
    }
    ...
    private static void buildSegment(ServerLevel level, Node a, Node b) {
        ...
        for (int step = 0; step <= steps; step++) {
            ...
            double rx = -dz / length, rz = dx / length;
            ...
            if (step % 28 == 0 && step > 8 && step < steps - 8) {
                lanternPost(level, cx + (int)Math.round(rx * 6), y, cz + (int)Math.round(rz * 6));
            }
        }
    }
    ...
    /** Fifth marker element invalidates the legacy ribbon marker and forces the integrated entry route to author once. */
    private static boolean hasMarker(ServerLevel level) {
        return level.getBlockState(new BlockPos(MARKER_X, MARKER_Y, MARKER_Z)).is(Blocks.LODESTONE)
                ...
                && level.getBlockState(new BlockPos(MARKER_X + 4, MARKER_Y, MARKER_Z)).is(Blocks.EMERALD_BLOCK);
    }
    ...
    private static void writeMarker(ServerLevel level) {
        ...
        set(level, MARKER_X + 4, MARKER_Y, MARKER_Z, Blocks.EMERALD_BLOCK);
    }
