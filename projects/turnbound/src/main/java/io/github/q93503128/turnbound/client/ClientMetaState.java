    ...
    public static void update(String raw) {
        ...
        for (String line : raw.split("\n")) {
            ...
            try {
                switch (p[0]) {
                    case "H" -> {
                        gold=Long.parseLong(p[1]); crystal=Long.parseLong(p[2]); essence=Long.parseLong(p[3]); core=Long.parseLong(p[4]);
                        cp=Integer.parseInt(p[5]); rift="1".equals(p[6]);
                        if (p.length>7) pity=Integer.parseInt(p[7]);
