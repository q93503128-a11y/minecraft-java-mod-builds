    ...
    void reportLineIsStableAndCarriesPersistedFieldPlayEvidence() {
        ...
        assertEquals(
            "run=7;owner=" + OWNER + ";status=deployed;endReason=none;elapsedTicks=240;salvage=2;pressure=3;plan=5[hunter=2,scout=2,elite=1]"
                + ";liveThreats=4;hazard=120t@2;hubSalvage=5;supply=6;nextCost=3;startContext=persisted;content=current;fingerprint=1234567890ab",
            snapshot.reportLine()
        );
    }

    @Test
    void terminalAndLegacySnapshotsExposeEvidenceQualityExplicitly() {
        FieldPlayReviewSnapshot snapshot = new FieldPlayReviewSnapshot(
            8L, OWNER, "failed", "server_restart", true, 400L, 3, 4,
            3, 2, 1, -1, 140, 1,
            8, 4, 4, "legacy-reconstructed", false, "abcdef"
        );
        ...
        assertTrue(snapshot.reportLine().contains("startContext=legacy-reconstructed"));
        ...
        assertThrows(IllegalArgumentException.class, () -> new FieldPlayReviewSnapshot(
            8L, OWNER, "failed", "server_restart", true, 400L, 3, 4,
            3, 2, 1, 0, 140, 1,
            8, 4, 4, "persisted", false, "abcdef"
        ));
        assertThrows(IllegalArgumentException.class, () -> new FieldPlayReviewSnapshot(
            9L, OWNER, "deployed", "player_logout", false, 12L, 0, 0,
            1, 1, 1, 3, 100, 0,
            0, 1, 1, "persisted", true, "abcdef"
        ));
        assertThrows(IllegalArgumentException.class, () -> new FieldPlayReviewSnapshot(
        ...
        assertThrows(IllegalArgumentException.class, () -> new FieldPlayReviewSnapshot(
            11L, OWNER, "deployed", "none", false, 12L, 0, 0,
            1, 1, 1, 3, 100, 0,
            0, 1, 1, "guessed", true, "abcdef"
        ));
    }
}
