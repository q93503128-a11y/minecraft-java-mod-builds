...
class FieldPlayReviewSnapshotTest {
    ...
    void reportLineIsStableAndCarriesFieldPlayEvidence() {
        ...
        assertEquals(
            "run=7;status=deployed;elapsedTicks=240;salvage=2;pressure=3;plan=5[hunter=2,scout=2,elite=1]"
    ...
    @Test
    void terminalSnapshotsDoNotPretendToKnowPostCleanupThreatCount() {
        FieldPlayReviewSnapshot snapshot = new FieldPlayReviewSnapshot(
            8L, "extracted", true, 400L, 3, 4,
            3, 2, 1, -1, 140, 1,
            8, 4, 4, false, "abcdef"
        );
