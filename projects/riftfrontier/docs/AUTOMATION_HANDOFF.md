...
`M3 — PLAYER COMBAT BUILD VERIFIED / COMMANDLESS TECHNICAL EXPEDITION LOOP BUILD VERIFIED / REGION 01 BOSS FIELD ROLE-CONTRAST + HEALTH + REVIEWED ATTACK/REACTION MOTION BUILD VERIFIED / PRODUCTION BOSS PRESENTATION STILL MATERIAL + ARENA-PRESSURE + VFX/SOUND GATED`
...
`ExpeditionHubTerminal.bootstrapFreshWorld(...)` now runs from the existing server login adapter **only when**:

- there is no active authoritative expedition; and
- the authoritative world has no expedition history at all.

It then:

- prepares the same bounded technical hub fixture near `(0, 100, 0)`;
- moves the player to that technical hub;
- exposes the existing temporary smithing-table provision station and lodestone deploy station;
- introduces no new SavedData fields, lifecycle state, balance values or station-owned economy.

The hub lodestone still delegates directly to `ExpeditionGameplayService.start(...)`; the smithing table still delegates directly to `ExpeditionGameplayService.provision(...)`. Existing expedition history prevents this fresh-world bootstrap from resetting or recreating established state on later logins.

`/riftfrontier expedition start` remains diagnostic/fallback only.

Code commits:

- `5f9d6b6bb889a5eab84135437df4150ec80c6494` — fresh-world technical hub bootstrap in `ExpeditionHubTerminal`;
- `19a54bdf9ac5c620099688cbdeac4e1722d3b21d` — login adapter exposes that bootstrap.

`Build Riftfrontier` workflow `34808226019`: **SUCCESS** through toolchain, asset-intake tests, `clean test build`, required GameTest, dedicated-server smoke, Xvfb client smoke, executable-JAR inspection, report generation and artifact uploads.

Deliverable `riftfrontier-0.1.0-alpha.1-deliverables` digest:

`sha256:e52d0cae1e0ed30c88b5e945c446c2c739dea013d3ed0b1f60b207781d327c35`

Treat this code checkpoint as `CODE REVIEWED / TESTED / BUILD VERIFIED / JAR PRODUCED` only. Human procedure is `docs/M2B_IN_WORLD_HUB_LOOP.md`, now beginning with fresh-world login -> technical hub -> hub lodestone rather than a command bootstrap.
...
Post-extraction loop checkpoint `09109a4b8fc11b5c136ff8d329e380fd9fe65eaf`, workflow `34782718258`: full Riftfrontier workflow SUCCESS.

No new lifecycle or persistence semantics were added by the fresh-world bootstrap. Death/logout/restart reconciliation, extraction gates, pressure-scaled preparation cost, storage, supply and evidence remain owned by their existing server-authoritative services.
...
- native `ServerBossEvent` reads real entity health; its neutral label is not final lore naming.
...
The commandless technical expedition loop is now closed enough for automated development to return to **production-visible Region 01 quality**. Do not use the new bootstrap as a reason to expand technical hub UI/backend scaffolding before reference-reviewed hub presentation work is actually scheduled.
...
Next automatic development should target:
...
Do **not** auto-tune provisional combat numbers without human field evidence. Do **not** promote native diagnostic cues, stone material, bossbar, lodestone, smithing table or technical hub platform as final art/UI.

## Verification vocabulary

Latest fresh-world expedition-loop code checkpoint:
