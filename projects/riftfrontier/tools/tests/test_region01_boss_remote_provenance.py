...
class Region01BossRemoteProvenanceTest(unittest.TestCase):
    """Offline regression gates for independently reproduced and visually reviewed Dragon Evolved evidence.

    Network reacquisition was a one-shot evidence operation. CI validates the deterministic machine audit separately
    from the later human visual-motion review so numerical provenance can still be regenerated byte-for-byte without
    erasing or pretending to generate human observations.
    """
    ...
    def test_committed_reacquisition_receipt_matches_canonical_contracts(self) -> None:
        ...
        # This is a deterministic machine-audit receipt. Human observations live in the separate visual-review receipt.
        self.assertEqual("NOT_TESTED", receipt["motion_semantics"]["status"])

    def test_visual_motion_review_is_pinned_to_exact_derivation_and_complete_inventory(self) -> None:
        project_root = Path(__file__).resolve().parents[2]
        review = json.loads(
            (project_root / "assets/sources/region_01_boss_dragon_evolved.motion_review.json").read_text(encoding="utf-8")
        )

        self.assertEqual("riftfrontier:region_01_boss_animation_motion_review", review["kind"])
        self.assertEqual(1, review["schema_version"])
        self.assertEqual(EXPECTED_SANITIZED_SHA256, review["accepted_derivation"]["sha256"])
        self.assertEqual(
            "assets/sources/region_01_boss_dragon_evolved.animation_audit.json",
            review["accepted_derivation"]["animation_audit_receipt"],
        )
        self.assertEqual([name for name, _ in EXPECTED_CLIPS], list(review["reviewed_clips"].keys()))
        self.assertEqual([0.0, 0.25, 0.5, 0.75, 1.0], review["method"]["sampled_normalized_times"])
        self.assertTrue(review["method"]["description"].strip())
        for observed_motion in review["reviewed_clips"].values():
            self.assertTrue(observed_motion.strip())
        self.assertEqual("NOT_AUTHORED", review["logical_binding"]["status"])
        self.assertIn("Do not derive gameplay mappings", review["logical_binding"]["note"])
