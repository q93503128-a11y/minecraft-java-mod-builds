...
| R01 | four Heartland catchable fish identities + Fish Codex icons | priority freshwater shortlist: CDmir Esox / CDmir Fish / Quaternius Armored Catfish; gfroad Catfish + joyfulsquirrel Fish alternates | `ASSET_INTAKE_ACTIVE` | old Quaternius clownfish/Sea-Life/tuna candidates rejected; acquire/hash freshwater binaries, inspect animation/scale/style, find only the missing fourth morphology if needed, then lock species names/icons |
