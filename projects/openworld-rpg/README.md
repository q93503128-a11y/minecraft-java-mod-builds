...
The latest verified technical state is workflow run `35943629696` at code state `bd9b339eb406000c74debb2082d1cad4abbf9c05`. Persistent combat Lv/EXP, independent per-root Class Rank/XP, Attributes, the canonical 12-slot equipped loadout, personal R01 progression, server-owned Gold, a four-slot Recovery Belt, a 36-slot Backpack, 36-slot Alderford Personal Storage, Material Pouch/Vault, Key Items, Pending Reward Claim and reconnect-safe reward plans are now persisted server-side. The normal R01 runtime now also idempotently projects the locked opening state: Heartland Arming Sword + Watch Buckler, one loaded Healing Potion and 150 Gold; first class selection has canonical Hunter/Bow, Cleric/Staff and Mage/Wand starter projections while Warrior/Guardian keep the opening sword+buckler. Better Combat melee, Spell Engine Arc Bolt and Bow projectile basics all replace donor damage with project authority for project-owned targets; Bow charge uses the actual server launch power after Ranged Weapon API pull-time handling rather than reverse-engineering donor damage. R01 now also has exact server reward rules for `Dust on the Quarry Road` and separate Earthloong first-boss / first-dungeon progression layers, while Earthloong item rolls and deterministic first-clear weapon choice remain unbound. A real joined-player playtest has already confirmed the authored Earthloong spawn path and Arc Bolt cast/release/impact path; broader R01 world/UI flow is still not playtested.
...
4. **Finish the remaining R01 authority connections without placeholders** — Arc Bolt real-player impact, Better Combat melee authority, Bow draw-power authority, persistent R01/Gold/Lv-EXP/Class-Rank state and reconnect-safe reward plans are now in source and CI-verified. `Dust on the Quarry Road` now activates only after the committed first-class/starter flow, rejects pre-activation credit, completes from three distinct legal action categories, pays the active class through reconnect-safe EXP/Class XP/Gold transactions and opens `Roots Below Stone`; login reconciliation closes the narrow quest-state→reward-plan crash window. Next bind the actual Lost Cargo/Viper/Marker/Gather/Roadside Trouble world triggers, Recovery Belt reload/use input and consume animation, player-facing dodge/guard input plus accepted full-body motion, and the physical item/icon + Lucifer inventory presentation. Earthloong first-clear item/choice delivery, save/reload/rejoin acceptance, multiplayer acceptance and Azari spatial binding remain incomplete.
