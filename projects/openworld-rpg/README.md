...
The narrow M0 technical implementation is now verified through workflow run `35558638544` at code state `eaed3b5fc2fd2ffddb4c5131f9cb0a91877d440f`. The project has a canonical player build→damage-snapshot producer and now also persists the first real combat-progression slice: combat Lv, optional active root class, and independent per-class VIT/END/STR/DEX/INT/WIL allocations live in a server-owned Fabric Data Attachment with Codec persistence/copy-on-death and strict point-budget/cap validation. A legal Lv8 Mage with +7 INT and ItemLv8 Staff produces `WeaponPower 30 / WeightedStat 10.95 / PoiseOutput 0.85`; the gameplay-server harness resolves **29 canonical damage / 4.25 poise** into exact Earthloong, yielding canonical HP **4900 → 4871**, proxy HP **1024 → 1017.9396**, and Poise **190 → 185.75**. The remaining player-authority gap is the real equipment inventory/affix publisher and automatic live build refresh: those are not implemented yet, so a real joined-player Arc Bolt and Better Combat hit remain untested rather than being faked with defaults.
...
4. **Finish the real-player M0 authority proof without placeholders** — canonical Earthloong HP/poise runtime, canonical build→snapshot math and persistent combat Lv/per-class Attribute state are now proven. Next implement the real equipment inventory/affix publisher + automatic live build refresh, then prove exactly-one joined-player Spell Engine and Better Combat damage before moving to Earthloong encounter/reward and registry-sync + quest save/reload/rejoin proofs.
