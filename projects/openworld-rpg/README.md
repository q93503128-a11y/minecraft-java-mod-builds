...
The narrow M0 technical implementation is now verified through workflow run `35557203570` at code state `8ddba64613ee837f35c64c14df82e21f7bd4ef65`. In addition to dependency/runtime containment, Better Combat/Spell Engine authority seams and project impact math, the exact Earthloong entity now has executable project-owned HP and poise runtime state. The server test exposed Minecraft's visible MAX_HEALTH clamp at **1024** versus canonical Earthloong **4900 HP**; this is now handled by canonical HP virtualization with proportional vanilla-health proxy synchronization. The runtime harness observes canonical HP **4900 → 4868** from one 32-damage Arc Bolt project transaction, proxy HP **1024 → 1017.3127**, Poise **190 → 185**, and the canonical **1.15x** break damage-taken window. Non-project HP damage to project-owned external actors fails closed. This still is not a joined-player spell playtest: production progression/equipment code does not yet produce a real player combat snapshot, so real player Spell Engine damage remains fail-closed rather than inventing stats.
