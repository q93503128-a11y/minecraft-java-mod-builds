...
The narrow M0 technical implementation is now verified through workflow run `35563533539` at code state `8c74f8ca37b1a63a3f6bb280bb23943f8ba90136`. Persistent progression, the canonical 12-slot equipped loadout and automatic live build publication remain in place, and the project now ships an actual Spell Engine `openworld_rpg:arc_bolt` resource rather than only a Java-side policy. The spell keeps donor cost neutral and routes impact through `openworld_rpg:project_impact`; its locked project range is 22 blocks, while the previously unspecified initial cast/projectile timing is 1.0 s / 1.0 blocks per tick from the current Wizards 26.2 Arcane Bolt reference. An opt-in local verification flag can prepare a joining real player as the legal Lv8 Mage +7 INT + ItemLv8 Staff build without affecting normal gameplay. CI still verifies exact Earthloong at 29 damage / 4.25 poise, canonical HP 4900→4871. A real human player has not yet fired the projectile in a joined world, so that result remains a manual acceptance test rather than a claimed playtest.
