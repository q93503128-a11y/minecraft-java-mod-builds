...
The latest verified technical state is workflow run `35852726146` at code state `fa75ef89d7204091811a7a7a4dc6eaffbc9930ec`. Persistent combat Lv/EXP, independent per-root Class Rank/XP, Attributes, the canonical 12-slot equipped loadout, personal R01 progression, server-owned Gold and reconnect-safe reward plans are now persisted server-side. Better Combat melee, Spell Engine Arc Bolt and Bow projectile basics all replace donor damage with project authority for project-owned targets; Bow charge uses the actual server launch power after Ranged Weapon API pull-time handling rather than reverse-engineering donor damage. R01 now also has exact server reward rules for `Dust on the Quarry Road` and separate Earthloong first-boss / first-dungeon progression layers, while Earthloong item rolls and deterministic first-clear weapon choice remain unbound. A real joined-player playtest has already confirmed the authored Earthloong spawn path and Arc Bolt cast/release/impact path; broader R01 world/UI flow is still not playtested.
...
4. **Finish the remaining R01 authority connections without placeholders** — Arc Bolt real-player impact, Better Combat melee authority, Bow draw-power authority, persistent R01/Gold/Lv-EXP/Class-Rank state and reconnect-safe reward plans are now in source and CI-verified. Next bind player-facing dodge/guard input and movement, starter equipment/item presentation, real R01 quest/world interaction triggers and class-attribution for quest/dungeon rewards, then finish Earthloong first-clear item/choice delivery plus save/reload/rejoin and multiplayer acceptance proofs.
