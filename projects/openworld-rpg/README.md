...
The narrow M0 technical implementation is now verified through workflow run `35558098504` at code state `0efbe653291c59228015ce445a33e696f8055e56`. The project now has a canonical server-domain producer for player damage-source snapshots: combat Lv, active root class, legal per-class Attribute allocation, VIT/END/STR/DEX/INT/WIL, weapon family + ItemLv, flat equipment stats, Physical/Magic/family Power and poise-output bonuses produce normalized WeaponPower, weighted stat, additive power and poise multiplier. Arc Bolt reads this domain and requires an active Mage build; missing player build state remains fail-closed. The runtime harness no longer hand-writes a generous offensive snapshot: a legal Lv8 Mage with +7 INT and an ItemLv8 Staff produces `WeaponPower 30 / WeightedStat 10.95`, then deals **29 canonical damage / 4.25 poise** to exact Earthloong, yielding canonical HP **4900 → 4871**, vanilla proxy **1024 → 1017.9396**, and Poise **190 → 185.75**. Production progression/equipment/save systems still do not publish real joined-player builds, so this is runtime authority proof rather than a player playtest.
...
4. **Finish the real-player M0 authority proof without placeholders** — canonical Earthloong HP/poise runtime is now proven. Next implement the real project Lv/attribute/equipment snapshot producer, bind it to actual joined-player Spell Engine and Better Combat actions, prove exactly-one player damage, then implement the Earthloong encounter/reward path and finish registry-sync + quest save/reload/rejoin proofs.
