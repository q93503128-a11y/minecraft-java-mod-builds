...
The narrow M0 technical implementation now exists and is verified through workflow run `35555391230` at code state `836657164f35f0988ad665d7119ce570d41d3f3d`: Fabric 26.2 / Java 25 toolchain, exact dependency/runtime profiles, integration validation, donor containment, Better Combat authority seam, Spell Engine Mana/cooldown admission, project impact math, Earthloong project target binding, an explicit no-fallback player combat snapshot seam, and the Minecraft-side project direct-magic final-damage applicator all build and boot together. The Arc Bolt policy now ignores donor spell power as damage authority and can resolve from project snapshots, but **real player source snapshots are not yet produced by progression/equipment code**, so normal runtime impact still fails closed before damage rather than inventing stats. A real Better Combat hit, real Arc Bolt → Earthloong hit, runtime poise application, R01 encounter/reward flow and player-facing R01 gameplay remain untested/unimplemented.
...
4. **Finish the bounded M0 runtime proof without player-facing placeholders** — bind the real project Lv/equipment/stat source snapshot, execute one Arc Bolt → Earthloong hit with exactly one expected HP delta, add runtime poise recovery/break application, then invoke the authored Earthloong encounter/reward path and finish registry-sync + quest save/reload/rejoin proofs. Better Combat real-hit execution remains a separate narrow regression.
