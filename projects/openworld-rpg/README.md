...
4. **Finish the remaining R01 authority connections without placeholders** — Arc Bolt real-player impact, Better Combat melee authority, Bow draw-power authority, persistent R01/Gold/Lv-EXP/Class-Rank state and reconnect-safe reward plans are now in source and CI-verified. Next bind player-facing dodge/guard input and accepted full-body motion, physical starter-item/inventory presentation, Recovery Belt use/input/consume animation, real R01 quest/world interaction triggers and class-attribution for quest/dungeon rewards, then finish Earthloong first-clear item/choice delivery plus save/reload/rejoin and multiplayer acceptance proofs. Azari spatial binding remains separate and incomplete.
