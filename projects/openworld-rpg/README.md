...
The narrow M0 technical implementation now exists: Fabric 26.2 / Java 25 toolchain, mod entrypoint, integration dependency manifest, runtime profiles, validation/overlay primitives, unit tests and CI. The pinned gameplay stack co-loads in the isolated `gameplay` dev runtime. Better Combat now also has the first project authority seam: commit `579354c1154c8c60ad534fe3c6558a6a57d33e66`, workflow run `35325649844`, binds the exact Better Combat 3.2.2 runtime interfaces, routes its primary melee damage through a project-owned server decision point, suppresses vanilla sweep damage during an active Better Combat attack, and reaches dedicated-server ready state. **A real player-to-target hit has not yet been executed in runtime**, so this is implementation/startup proof rather than playtest proof. Spell Engine authority, creature ecology/stat/loot overrides and R01 gameplay remain open.
...
4. **Continue M0 authority-bridge proof without player-facing placeholders** — Better Combat now has a code/startup-verified project authority seam, while real-hit execution remains untested. Next implement the bounded Spell Engine bridge: project Mana/cooldown eligibility plus exactly one project impact transaction; then bind one accepted external creature overlay with donor spawn/stat/loot suppression. A narrow Better Combat real-hit regression should be added when a safe automated target harness exists or during the first client/playable combat test.
