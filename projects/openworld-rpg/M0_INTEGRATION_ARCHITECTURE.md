...
The dependency runtime gate now includes external-actor HP/poise authority, persistent combat progression, a persistent canonical equipped-loadout publisher, packaged Spell Engine data for `openworld_rpg:arc_bolt`, a project-owned Better Combat melee transaction for bound actors, canonical player MaxHP/VIT projection and the first END/Stamina runtime. The dedicated M0 playtest artifact has been manually world-joined: authored Earthloong spawn, Arc Bolt cast/release, single-press behavior, cooldown presentation and a landed Arc Bolt impact are PLAYTESTED in that narrow scope. CI run `35689727920` verifies unit/resource tests, clean build/JAR, core/gameplay server boot, exact Earthloong Arc Bolt runtime proof, the direct-physical damage bridge, a canonical Lv8 Sword/+7 STR melee fixture where donor proposal 9999 is ignored and the project applies 17 damage / 10 Poise (HP 4900→4883, Poise 190→180), locked MaxHP/VIT reference anchors, primary gear-stat aggregation without requiring a weapon, canonical Stamina/END anchors, 5 Stamina/s sprint drain, the exact 7-tick post-sprint regeneration boundary, canonical active-defense timing/math, and persistent armor/shield defensive-equipment publication. Better Combat 26.2 still supplies attack presentation/hit candidates; project family cadence/cycle rules determine canonical basic-hit damage. Player-vitals/Stamina runtime wiring is backend/unit/build/startup verified but not yet manually world-joined. Canonical dodge Stamina/i-frame/re-entry timing, hold-guard/perfect-guard timing, guard-impact Stamina pressure, guard break, and player incoming Defense/MR/routine-mitigation authority are now project-owned domain state. Persistent equipment now also publishes the player defensive snapshot independently from the offensive weapon build: per-slot-rounded armor Defense/MR, shield-family GuardRating, and Defense/MR/Guard Strength affix aggregation are project-owned. Weapon guard remains intentionally unbound until canon supplies a real weapon GuardRating. A normalized project incoming-hit runtime bridge is now present, and donor-origin HP damage from project-owned external actors fails closed against players. Final player-facing input/movement/animation and attack-specific encounter hit-event binding remain unbound; no temporary Q-key roll or donor combat authority is substituted. The remaining Better Combat gate is a real joined-player Better Combat network/presentation hit, not the server damage formula.
...
PLAYER ACTIVE-DEFENSE AUTHORITY: IMPLEMENTED + UNIT/BUILD/STARTUP VERIFIED — dodge/guard/perfect-guard/guard-break/Stamina + incoming DEF/MR core; perfect-guard-only committed attacks supported; player-facing input/animation NOT IMPLEMENTED
PLAYER DEFENSIVE EQUIPMENT PUBLISHER: IMPLEMENTED + UNIT/BUILD/STARTUP VERIFIED — per-slot-rounded armor DEF/MR + shield-family GuardRating + Defense/MR/Guard Strength affix aggregation; weapon guard rating intentionally UNBOUND; run 35689727920
PLAYER INCOMING DAMAGE BRIDGE: IMPLEMENTED + BUILD/STARTUP VERIFIED — explicit normalized project hits resolve Defense/MR/dodge/guard/perfect-guard/Stamina before HP application; project-owned external-actor donor HP damage to players FAIL-CLOSED; Earthloong Phase-1 Claw/Tail/Quarry canonical physical impact data + authority IMPLEMENTED; Claw/Tail/Quarry start geometry IMPLEMENTED + UNIT VERIFIED; Claw/Quarry donor presentation-state bridge RUNTIME VERIFIED; Claw/Quarry project-owned telegraph/action/recovery scheduling, committed movement and contact invocation IMPLEMENTED + BUILD/SERVER/CLIENT-STARTUP VERIFIED; Tail final presentation remains UNBOUND
...
EXTERNAL CREATURE PROJECT BINDING FOUNDATION: PARTIAL — Earthloong registry/overlay/authored spawn/donor XP+loot suppression, canonical 4900 HP virtualization, DEF/MR and runtime Poise/break state verified; action-selection controller + Phase-1 Claw/Tail/Quarry impact authority + physical start geometry are implemented; Claw/Quarry project-owned scheduling/movement/contact runtime and donor presentation states are verified through build/server/client-startup gates; Root Breaker project timing/radius/direct-impact/tagged-prop runtime is implemented with technical donor-roar presentation, while player-poise application and final VFX remain open; donor baseTick skill selection is target-gated by the project; Tail final presentation, Lightning Furrow final lane geometry/runtime, remaining Phase-2 magic/area impacts and project reward transaction remain
...
BUILD VERIFIED: YES — latest Earthloong Phase-1 runtime run 35698061263
JAR PRODUCED: YES
