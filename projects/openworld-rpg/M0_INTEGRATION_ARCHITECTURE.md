...
The dependency runtime gate now includes real external-actor HP/poise runtime authority. The 10 foundation/safety JARs plus three curated creature JARs remain pinned and co-load in Fabric 26.2. Better Combat still has a project-owned admission seam, while real player melee execution is not yet tested. Spell Engine has project-owned Mana/cooldown admission plus canonical direct-magic resolution; production project spell damage still requires a real server-owned player combat snapshot and therefore fails closed when progression/equipment authority is absent. Spell Engine `enginePower`/impact-total inputs are not damage authority. For exact actor `threateningly_mobs:the_earthloong`, canonical HP is **4900** even though Minecraft clamps the visible MAX_HEALTH proxy to **1024**. `ProjectHealthRuntimeState` owns canonical HP and keeps vanilla health proportional; external-actor non-project HP damage is blocked by one-shot project damage authorization. Runtime boss poise is bound with the canonical 6 s recovery delay, 12.5%/s recovery, 2.4 s break, +15% direct-damage-taken break window and 1.5 s post-break 50% poise-damage reduction. Gameplay-server verification in run `35557203570` at code state `8ddba64613ee837f35c64c14df82e21f7bd4ef65` used the exact Earthloong entity and production impact/application code: canonical HP **4900 → 4868**, vanilla proxy **1024 → 1017.3127**, Poise **190 → 185**, break damage-taken multiplier **1.15**. This is an automated runtime fixture, not a claim that a joined player cast Arc Bolt: the fixture supplies the offensive source snapshot because the production progression/equipment subsystem does not yet exist. Donor spell/equipment/loot/ecology containment and exact Earthloong registry validation continue to pass. The next M0 work is the real player source snapshot producer and real Spell Engine/Better Combat player execution proof, then the authored Earthloong encounter/reward path, registry synchronization and quest save/reload/rejoin acceptance proofs. Player-facing R01 implementation still waits on the remaining asset/spatial gates.
...
PROJECT DIRECT-MAGIC IMPACT BRIDGE: PARTIAL — canonical resolver + exact Earthloong Minecraft HP runtime application verified; real player source snapshot and real Spell Engine player hit remain
...
BUILD VERIFIED: YES — run 35557203570
JAR PRODUCED: YES
