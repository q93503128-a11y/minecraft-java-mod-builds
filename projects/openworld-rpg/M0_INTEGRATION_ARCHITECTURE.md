...
The dependency runtime gate now includes both external-actor HP/poise runtime authority and a canonical player-build-to-damage-source producer. The player combat build domain owns combat Lv, active root class, per-class Attribute allocation, VIT/END/STR/DEX/INT/WIL, equipped weapon family/ItemLv, flat equipment stats, Physical/Magic/weapon-family power bonuses and poise-output bonuses. Validation enforces `earned points = Lv - 1` as the maximum spent pool and pre-equipment stat value <=60. The producer derives normalized WeaponPower from ItemLv + family factor, weighted offensive stat from the canonical family weights, school-specific additive power, and the weapon-family poise multiplier. Spell Engine Arc Bolt consumes that produced Magic snapshot only for an active Mage; missing/wrong-class build state is fail-closed. This is production authority structure, but the progression/equipment inventory/save systems that will publish real joined-player builds are still pending, so there is no bootstrap weapon or fabricated player loadout. Workflow run `35558098504` at `0efbe653291c59228015ce445a33e696f8055e56` replaces the older arbitrary source fixture with a canon-valid Lv8 Mage / +7 INT / ItemLv8 Staff build. Observed exact-Earthloong runtime result: **WeaponPower 30, weighted stat 10.95, 29 damage, canonical HP 4900 → 4871, vanilla HP proxy 1024 → 1017.9396, Poise 190 → 185.75, break damage-taken multiplier 1.15**. Donor Spell Engine power/impact-total values do not own that result. Better Combat remains at the existing neutral server proposal seam because its exact visible attack-cycle coefficient/hit partition has not yet been extracted into project authority; external project actors reject that unowned HP path rather than accepting incorrect melee math. The next M0 work is to implement persistent real player progression/class-allocation/equipment state that publishes this build, then prove actual joined-player Spell Engine and Better Combat actions without duplicate/donor damage, followed by Earthloong encounter/reward, registry sync and quest persistence/rejoin proofs.
