...
The dependency runtime gate is now materially further: the 10 foundation/safety JARs plus the three curated creature JARs are pinned by exact admitted versions and SHA-256 and co-load in the Fabric 26.2 `gameplay` profile. Normal launch defaults to `gameplay`; `core` remains an explicit isolation profile. The full 13-dimension actor-overlay policy is structurally required. Better Combat retains the project-owned melee damage seam, is conditionally mixed in only when present, and has donor fallback weapon assignment/vanilla sweeping contained; a real target hit is still **NOT TESTED**. Spell Engine has project-owned resource/cooldown admission plus a canonical direct-magic impact policy. Project spell damage now requires an explicit server-owned player combat snapshot and an explicit project target snapshot; there is no default player snapshot, so missing progression/equipment authority remains fail-closed. Spell Engine `enginePower` and impact-total values are observed only as donor callback inputs and are not used as project damage authority. An already-resolved final magic amount can now be applied through the project `project_direct_magic` Minecraft damage type without vanilla armor/effect/enchantment/resistance/shield/cooldown/knockback modifying it a second time; invulnerability/creative protection is intentionally not bypassed. Persistent runtime poise-state application is not yet bound even though the pure resolver computes canonical poise pressure. Donor spell-book creation/unbinding, item/durability/exhaust costs, fallback weapon spell assignment and RPG-Series loot injection remain disabled for the project runtime. MobFilter rejects ordinary natural/worldgen/spawner/structure/etc. random population channels, reserving explicit project-authored spawn paths. The first real actor binding validates and resolves `threateningly_mobs:the_earthloong` at server start, applies canonical R01 4900 HP, exposes DEF 45 / MR 35 / Poise 190 to the project target snapshot, provides an authored spawn path, marks capture suppression and suppresses donor XP/loot. Workflow run `35555391230` at code state `836657164f35f0988ad665d7119ce570d41d3f3d` verifies exact dependency hashes, unit tests, clean build/JAR, core/gameplay dedicated-server boot, the delayed exact Earthloong registry gate, donor-containment checks and gameplay client render-thread startup. A **real player Arc Bolt hit is still NOT TESTED** because no production progression/equipment subsystem yet binds the source snapshot. The next M0 work is to bind that real source snapshot, prove exactly one Arc Bolt → Earthloong HP change and runtime poise handling, then invoke the authored encounter/reward path and finish registry-sync plus quest save/reload/rejoin acceptance proofs. Player-facing R01 implementation still waits for the remaining asset/spatial gates.
...
PROJECT DIRECT-MAGIC IMPACT BRIDGE: PARTIAL — canonical source/target snapshot resolver + Minecraft final-damage applicator implemented/unit-built/startup verified; real player source snapshot binding, runtime poise state and real hit remain
EXTERNAL CREATURE PROJECT BINDING FOUNDATION: PARTIAL — Earthloong registry/overlay/HP/authored-spawn utility/donor XP+loot suppression and DEF/MR/Poise resolver input verified; encounter invocation, runtime poise application and project reward transaction remain
...
BUILD VERIFIED: YES — run 35555391230
JAR PRODUCED: YES
