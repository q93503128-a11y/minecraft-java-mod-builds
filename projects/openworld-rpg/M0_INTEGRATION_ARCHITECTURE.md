...
The dependency runtime gate now includes external-actor HP/poise authority, persistent combat progression and a persistent canonical equipped-loadout publisher. The project persists exactly the 12 canonical equipped slots and aggregates project-owned ItemLv/weapon-family/affix data into `EquipmentCombatState`; missing Main Weapon removes combat authority instead of inventing a fallback. The publisher applies the canonical family weights/power factors, matching-family power cap (+60%), and `wand + focus` formula (`FocusPower = WeaponBudget(focus ItemLv) * 0.18`) while preventing a two-handed staff from simultaneously using a magical focus. `PlayerCombatBuildPublisher` refreshes the transient server combat build on join and on project progression/equipment mutations. Spell Engine Arc Bolt consumes that produced build only for active Mage. Full backpack/loot generation/UI are outside this M0 loadout slice and remain unimplemented. Workflow run `35560853804` at `a89c8393e5c4eafcc7397a67447eed65da9774af` passes unit tests, clean build/JAR, core/gameplay server boot, exact Earthloong runtime damage/poise proof and client startup smoke. Observed fixture remains WeaponPower 30 / WeightedStat 10.95 / 29 damage / 4.25 poise, Earthloong canonical HP 4900→4871 and Poise 190→185.75. No CI player actually joins, so real joined-player Spell Engine and Better Combat execution remain untested. Better Combat additionally remains at the neutral proposal seam until its exact visible attack-cycle coefficient and hit partition are bound to project authority. Next M0 work is real-player execution proof, then Earthloong encounter/reward, registry sync and quest persistence/rejoin acceptance.
...
BUILD VERIFIED: YES — run 35560853804
JAR PRODUCED: YES
