...
- 10 foundation/safety JARs + 3 curated creature JARs are pinned to exact admitted versions/bytes and co-load in the `gameplay` profile. Workflow run `35560853804` verifies unit tests, clean build/JAR, core dedicated-server boot, gameplay dedicated-server boot, donor-containment startup rules, the R01 Earthloong exact registry/profile binding at `SERVER_STARTING`, persistent combat progression, the persistent 12-slot equipped-loadout combat publisher, canonical-HP/poise runtime verification harness, and a headless gameplay-client render-thread startup smoke at Openworld code state `a89c8393e5c4eafcc7397a67447eed65da9774af`.
...
The narrow M0 source/bootstrap and dependency-runtime layer now includes persistent progression **and persistent equipped-loadout authority**. `PlayerProgressionState` owns combat Lv, optional active root class and independent per-root-class Attribute allocations. `PlayerEquipmentLoadoutState` owns the canonical 12 equipped slots (`Main Weapon / Off-hand / Head / Chest / Legs / Gloves / Boots / Necklace / Ring 1 / Ring 2 / Charm / Relic`) as a persistent Fabric Data Attachment with copy-on-death semantics. Equipped items carry project item identity, ItemLv, active weapon family where applicable, magical-focus identity and combat-relevant affixes. The loadout publisher aggregates flat primary stats, Physical/Magic Power, matching weapon-family power with the canonical +60% gear cap, authored poise output and the exact `wand + focus` supplemental magic WeaponPower rule; a two-handed staff cannot simultaneously use a magical focus. Missing Main Weapon produces no combat equipment authority rather than a fallback weapon. `PlayerCombatBuildPublisher` rebuilds the transient authoritative combat build on server join, every project progression/class/stat mutation and every project equip/unequip mutation; incomplete state removes the live build. Arc Bolt continues to require an active Mage build and consumes only that published project snapshot. Full backpack/storage, loot generation, ordinary equipment acquisition UI and complete affix roster are **not** claimed implemented by this M0 equipped-loadout slice. The gameplay harness still uses the canon-valid Lv8 Mage / +7 INT / ItemLv8 Staff and verifies exact Earthloong runtime values: WeaponPower **30**, WeightedStat **10.95**, damage **29**, canonical HP **4900 → 4871**, proxy HP **1024 → 1017.9396**, Poise **190 → 185.75**, break damage-taken multiplier **1.15**. Workflow run `35560853804` at code state `a89c8393e5c4eafcc7397a67447eed65da9774af` passes exact dependency checks, unit tests including loadout Codec/slot/affix/wand+focus/cap rules, clean build/JAR, core server, gameplay server runtime proof and client startup smoke. The remaining M0 player-authority gap is now actual joined-player execution: no automated CI player joins the world, so real joined-player Arc Bolt and Better Combat melee remain **NOT TESTED**. Better Combat also still needs its exact project attack-cycle coefficient/hit partition before its donor proposal amount can be replaced correctly. After that come Earthloong encounter/reward, registry-sync and quest save/reload/rejoin proofs.
