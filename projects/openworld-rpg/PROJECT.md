...
- 10 foundation/safety JARs + 3 curated creature JARs are pinned to exact admitted versions/bytes and co-load in the `gameplay` profile. Workflow run `35557203570` verifies unit tests, clean build/JAR, core dedicated-server boot, gameplay dedicated-server boot, donor-containment startup rules, the R01 Earthloong exact registry/profile binding at `SERVER_STARTING`, the canonical-HP/poise runtime verification harness, and a headless gameplay-client render-thread startup smoke at Openworld code state `8ddba64613ee837f35c64c14df82e21f7bd4ef65`.
...
The narrow M0 source/bootstrap and dependency-runtime layer now exists: Fabric toolchain, mod entrypoint, exact dependency/version manifest, `core/gameplay/essential` profiles, validation primitives, integration registry, actor-overlay schema/validation, unit tests and project CI. The actor-overlay validator requires the full canonical 13-dimension policy surface. Normal runtime defaults to `gameplay`; `core` is isolation-only. Better Combat retains a project-owned server damage admission seam, but **real player-to-target Better Combat execution remains NOT TESTED** and external actors now fail closed against non-project HP damage until canonical melee resolution is bound. Spell Engine owns cast/target/delivery infrastructure while project Mana/cooldown and project impact authority remain server-owned. `PlayerCombatSnapshotStore` still has **no fallback/default offensive snapshot**: production player damage cannot borrow Spell Engine `enginePower`, vanilla attack damage or invented bootstrap stats. Arc Bolt's direct-magic policy consumes only project source/target snapshots and ignores donor power/impact-total values for damage authority. The first real external actor remains exact registry id `threateningly_mobs:the_earthloong`, with canonical R01 **4900 HP / DEF 45 / MR 35 / Poise 190**. Runtime verification exposed that Minecraft clamps the visible MAX_HEALTH value to **1024** for this actor, so the project no longer treats vanilla health as authoritative: `ProjectHealthRuntimeState` owns canonical HP and vanilla health is synchronized as a proportional proxy. Run `35557203570` proves a canonical Arc Bolt fixture resolves **32 damage**, changes Earthloong canonical HP **4900 → 4868**, and synchronizes the Minecraft proxy **1024 → 1017.3127**. External-actor vanilla/donor HP damage is fail-closed through a one-shot project damage authorization so proxy HP cannot diverge from canonical HP through an unowned damage path. Runtime boss poise is also bound: the same Arc Bolt fixture changes **190 → 185 Poise**; boss recovery starts after 6 s at 12.5% PoiseMax/s; break lasts 2.4 s with **+15% direct damage taken**; recovery from break refills poise and grants 1.5 s of **50% poise-damage reduction**. Unit tests cover health virtualization, poise timing/break protection, the sub-5 offensive-stat curve, donor-power rejection and missing-player-snapshot fail-closed behavior. The CI harness uses the exact Earthloong registry entity and production resolver/damage/poise code, but its offensive source snapshot and attacker are explicit verification fixtures; therefore **a real joined player Arc Bolt through the Spell Engine callback is still NOT TESTED**. The production progression/equipment subsystem still does not exist, so no real player source snapshot producer exists yet. Earthloong's authored encounter controller and replacement project reward transaction also remain pending. Workflow run `35557203570` at code state `8ddba64613ee837f35c64c14df82e21f7bd4ef65` passes exact dependency verification, unit tests, clean build/JAR, core server, gameplay server with canonical HP/poise runtime proof, and gameplay client startup smoke. Remaining M0 work is the real progression/equipment source snapshot, real player Arc Bolt/Better Combat execution proof, Earthloong encounter/reward path, registry-sync proof and quest save/reload/rejoin proof.
