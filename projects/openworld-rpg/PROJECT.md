...
- 10 foundation/safety JARs + 3 curated creature JARs are pinned to exact admitted versions/bytes and co-load in the `gameplay` profile. Latest completed Openworld verification run `35943629696` at code state `bd9b339eb406000c74debb2082d1cad4abbf9c05` passes unit tests, clean build/JAR, core dedicated-server boot, gameplay dedicated-server boot and gameplay-client startup smoke. The verified R01 authority layer now includes the canonical opening/class-starter flow, persistent four-slot Recovery Belt, server-owned 36-slot Backpack + 36-slot Alderford Personal Storage + Material Pouch/Vault + Key Items + Pending Reward Claim, safe displaced-starter preservation, Pouch-first settlement material sourcing, and the `Dust on the Quarry Road` three-distinct-action completion/reward transition into `Roots Below Stone`, including reconnect reconciliation for the quest-state→reward-plan commit window.
