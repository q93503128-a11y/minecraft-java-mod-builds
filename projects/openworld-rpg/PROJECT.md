...
- 10 foundation/safety JARs + 3 curated creature JARs are pinned to exact admitted versions/bytes and co-load in the `gameplay` profile. Latest verification run `35672284552` passes unit tests, clean build/JAR, core dedicated-server boot, gameplay dedicated-server boot, donor-containment startup rules, exact Earthloong registry/runtime proof, canonical Arc Bolt and Better Combat bound-actor damage bridges, player MaxHP/VIT and Stamina/END domain tests, and gameplay-client startup smoke at Openworld code state `77b6591c97ed42342d2b38f7acf045a44b28990c`.
...
The narrow M0 source/bootstrap and dependency-runtime layer now includes persistent progression, persistent equipped-loadout authority, the first **actual Spell Engine spell resource** for `openworld_rpg:arc_bolt`, canonical Better Combat melee replacement for project-owned actors, and the first project-owned player-vitals/resource foundation. Arc Bolt keeps the locked 22-block range, project CUSTOM impact handler and donor-neutral cost; its current 1.0 s cast / 1.0 blocks-per-tick projectile timing remains playtest-tunable while damage, Mana, cooldown and Poise stay project-owned. The dedicated `-m0-playtest.jar` prepares a legal Lv8 Mage +7 INT / ItemLv8 Staff test build and exposes only marker-gated integration commands; the normal JAR remains free of that verification marker. A real joined-player playtest has confirmed the authored Earthloong spawn path, Arc Bolt cast/release, held-input behavior, project cooldown presentation and a successful Arc Bolt impact without the target disappearing or corrupting. CI run `35672284552` verifies the canonical Better Combat melee foundation: donor proposed damage is ignored for a bound actor, the project family cadence/cycle budget drives the hit, and an exact Lv8 Sword/+7 STR fixture applies 17 physical damage / 10 Poise to Earthloong (canonical HP 4900→4883, Poise 190→180) through the project direct-physical bridge. The same run verifies the canonical player MaxHP/VIT formula and primary equipment-stat aggregation plus the END/Stamina formula, 5 Stamina/s sprint drain, exact 7-tick post-sprint regeneration delay and percentage-preserving resource resynchronization. Player MaxHP is server-projected from canonical level/VIT, natural HP recovery is wired at 0.40% MaxHP/s after the 8 s hostile/combat quiet delay, and vanilla hunger-driven recovery is suppressed. These player-vitals/Stamina paths are **backend/unit/build/startup verified, not yet real-world-joined playtested**; dodge/guard/parry Stamina consumers are still separate future connections. Better Combat's real joined-player network/presentation hit also still requires one manual acceptance pass before M0 gate 9 can be called closed.
