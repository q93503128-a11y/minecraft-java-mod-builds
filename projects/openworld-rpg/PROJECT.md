...
- 10 foundation/safety JARs + 3 curated creature JARs are pinned to exact admitted versions/bytes and co-load in the `gameplay` profile. Workflow run `35558098504` verifies unit tests, clean build/JAR, core dedicated-server boot, gameplay dedicated-server boot, donor-containment startup rules, the R01 Earthloong exact registry/profile binding at `SERVER_STARTING`, the canonical player-build snapshot producer, canonical-HP/poise runtime verification harness, and a headless gameplay-client render-thread startup smoke at Openworld code state `0efbe653291c59228015ce445a33e696f8055e56`.
...
The narrow M0 source/bootstrap and dependency-runtime layer now exists: Fabric toolchain, exact dependency/profile validation, donor containment, external-actor authority, Better Combat/Spell Engine seams, canonical project impact math, and server-owned player combat build production. The current build domain now represents **combat Lv, active root class, canonical per-class Attribute Point allocation, all six attributes (VIT/END/STR/DEX/INT/WIL), equipped weapon family + Item Lv, flat equipment stat bonuses, Physical/Magic Power, weapon-family power, and poise-output bonus**. It enforces Lv-earned Attribute Point limits and the pre-equipment 60-stat cap, then derives normalized WeaponPower, weighted offensive stat, additive power and weapon-family poise multiplier from the canonical combat/equipment formulas instead of accepting donor/vanilla combat numbers. All admitted weapon families use the exact family power/poise/stat-weight data from `COMBAT_BALANCE.md` / `EQUIPMENT_BALANCE.md`. The Spell Engine Arc Bolt impact path now reads this authoritative build domain and additionally requires the active root class to be Mage; a missing build or wrong active class fails closed. **No production progression/equipment inventory system publishes these builds yet**, so ordinary joined players still do not receive an invented combat snapshot. The old hand-authored M0 damage fixture has also been replaced: runtime verification now uses a canon-valid **Lv8 Mage, 7 earned Attribute Points spent into INT, ItemLv8 Staff, no bonus affixes**. The producer yields **WeaponPower 30 / weighted stat 10.95 / poise output 0.85**; against exact Earthloong MR 35 the Arc Bolt project transaction resolves **29 damage / 4.25 poise**, changing canonical HP **4900 → 4871**, vanilla proxy HP **1024 → 1017.9396**, and Poise **190 → 185.75**. The donor callback still supplies absurd `enginePower=9999` in the verification path without changing this project result. Earthloong canonical HP virtualization, non-project external-actor damage rejection, 6 s boss-poise recovery, 2.4 s break, +15% break damage-taken window and post-break 50% poise protection remain runtime verified. Workflow run `35558098504` at code state `0efbe653291c59228015ce445a33e696f8055e56` passes dependency verification, unit tests, clean build/JAR, core server, gameplay server with produced-source runtime impact proof, and gameplay client startup smoke. **A real joined-player Arc Bolt callback/hit and real Better Combat player hit remain NOT TESTED**, because persistence/progression/equipment mutation and actual player equipment binding are the next authority layer. Earthloong encounter-controller/reward, registry-sync and quest save/reload/rejoin proofs also remain.
