...
- 10 foundation/safety JARs + 3 curated creature JARs are admitted to the isolated `gameplay` dev runtime and dedicated-server co-load verified at code state `d35658c21bde609001e6d440ef0089be058b74b7`, workflow run `35313724183`; this is dependency/runtime proof, not finished gameplay adapters
...
The narrow M0 source/bootstrap and dependency-runtime layer now exists: Fabric toolchain, mod entrypoint, integration dependency manifest, `core/gameplay/essential` profiles, validation primitives, integration registry, actor-overlay schema/validation, unit tests and project CI. The `gameplay` profile co-loads the current 10 foundation/safety JARs plus 3 curated creature JARs and reaches dedicated-server ready state; see `M0_GAMEPLAY_RUNTIME_BOOT_2026-09-18.md`. Better Combat melee reaches a project-owned server damage decision before the one canonical primary damage application, and vanilla sweep damage is suppressed while a Better Combat attack is active. **Real player-to-target Better Combat hit execution is still NOT TESTED.** Spell Engine now has a real project-owned resource/cooldown transaction seam. Canonical `Arc Bolt` authority uses the authored 12 Mana / 60-tick cooldown contract, WIL-driven MaxMana/regeneration, the 20-tick post-spend regen lock, the 100-tick out-of-combat recovery threshold, PRE side-effect-free eligibility, POST one-time resource/cooldown commit, cross-spell accepted-cast serialization, delayed projectile-impact lifetime separation, and runtime rejection of non-neutral donor exhaust/durability/item/effect/cooldown costs. The Spell Engine `CUSTOM` impact hook is registered but remains deliberately fail-closed until project combat stats/equipment/Defense/MR/poise resolution exists; no donor damage formula is silently accepted. Openworld RPG CI run `35545779733` at commit `7b831bb8cf87a598be515997bf6dfd1750066140` verifies the finalized shared combat-state serialization and cross-spell regression on core/gameplay dedicated-server profiles. Remaining M0 work is the real project impact/damage/heal/status transaction, MobFilter ecology rules and one external-creature spawn/stat/loot override.
