...
- 10 foundation/safety JARs + 3 curated creature JARs are pinned to exact admitted versions/bytes and co-load in the `gameplay` profile. Workflow run `35552108997` verifies unit tests, clean build/JAR, core dedicated-server boot, gameplay dedicated-server boot, donor-containment startup rules, the R01 Earthloong registry/profile binding, and a headless gameplay-client render-thread startup smoke at code state `870f6881db4625d3a7795e403dcf9d2fd344bf46` (the following `ab3807d0cc3ba6ba57d92d2573810f751f32abaa` has the same repository tree).
...
The narrow M0 source/bootstrap and dependency-runtime layer now exists: Fabric toolchain, mod entrypoint, exact dependency/version manifest, `core/gameplay/essential` profiles, validation primitives, integration registry, actor-overlay schema/validation, unit tests and project CI. The actor-overlay validator requires the full canonical 13-dimension policy surface, including movement/combat AI, recipes/worldgen and capture/duplication ownership. Normal runtime defaults to `gameplay`; `core` is isolation-only. Better Combat melee reaches a project-owned server damage decision before the one canonical primary damage application, the project mixin is conditionally applied only when Better Combat is present, and donor fallback weapon assignment/vanilla sweeping are contained. **Real player-to-target Better Combat hit execution is still NOT TESTED.** Spell Engine has a project-owned resource/cooldown transaction seam with cast-process identity so long casts cannot double-spend or become falsely rejected after the short duplicate-callback window. Canonical `Arc Bolt` still uses 12 Mana / 60 ticks; WIL changes preserve Mana percentage instead of creating a free refill; combat activity extends the out-of-combat regeneration threshold; disconnect removes transient combat state. Donor spell-book creation/unbinding, donor item/durability/exhaust costs, weapon fallback and RPG-Series loot injection are disabled at startup for the gameplay profiles. The `CUSTOM` impact route remains deliberately fail-closed until project combat stats/equipment/Defense/MR/poise resolution exists. MobFilter loads a project-managed ecology rule that suppresses donor/vanilla random authored-world spawn channels while leaving explicit project-owned spawn paths available. The first real external-actor binding now targets exact registry id `threateningly_mobs:the_earthloong`: the project validates the 13-dimension overlay at startup, owns the authored spawn utility, applies the canonical R01 **4900 HP**, exposes **DEF 45 / MR 35 / Poise 190** as the project combat profile, tags capture suppression, and suppresses donor XP plus the donor entity loot table. DEF/MR/Poise are not yet consumed by final damage resolution, the authored spawn utility has not yet been invoked by an encounter controller, and the replacement project reward transaction is still pending; those are intentionally not claimed as complete. Workflow run `35552108997` verifies exact dependency hashes, unit tests, clean build/JAR, core server boot, gameplay server boot including the exact Earthloong registry binding, containment checks and a headless client render-thread startup smoke. Remaining M0 work is now centered on the real project impact/damage/heal/status transaction, then consuming the Earthloong profile/reward path in a real encounter, registry sync proof and quest save/reload/rejoin proof.
