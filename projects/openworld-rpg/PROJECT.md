...
- 10 foundation/safety JARs + 3 curated creature JARs are pinned to exact admitted versions/bytes and co-load in the `gameplay` profile. Workflow run `35549006979` verifies unit tests, clean build/JAR, core dedicated-server boot, gameplay dedicated-server boot, donor-containment startup rules and a headless gameplay-client render-thread startup smoke at commit `f0f6c0d22ba564d7ed0ad92551889c7e209d0d8e`.
...
The narrow M0 source/bootstrap and dependency-runtime layer now exists: Fabric toolchain, mod entrypoint, exact dependency/version manifest, `core/gameplay/essential` profiles, validation primitives, integration registry, actor-overlay schema/validation, unit tests and project CI. The actor-overlay validator now requires the full canonical 13-dimension policy surface, including movement/combat AI, recipes/worldgen and capture/duplication ownership. Normal runtime defaults to `gameplay`; `core` is isolation-only. Better Combat melee reaches a project-owned server damage decision before the one canonical primary damage application, the project mixin is conditionally applied only when Better Combat is present, and donor fallback weapon assignment/vanilla sweeping are contained. **Real player-to-target Better Combat hit execution is still NOT TESTED.** Spell Engine has a project-owned resource/cooldown transaction seam with cast-process identity so long casts cannot double-spend or become falsely rejected after the short duplicate-callback window. Canonical `Arc Bolt` still uses 12 Mana / 60 ticks; WIL changes preserve Mana percentage instead of creating a free refill; combat activity now extends the out-of-combat regeneration threshold; disconnect removes transient combat state. Donor spell-book creation/unbinding, donor item/durability/exhaust costs, weapon fallback and RPG-Series loot injection are disabled at startup for the gameplay profiles. The `CUSTOM` impact route remains deliberately fail-closed until project combat stats/equipment/Defense/MR/poise resolution exists. MobFilter now loads a project-managed ecology rule that suppresses donor/vanilla random authored-world spawn channels while leaving command/item-use paths available for project-owned spawning. Workflow run `35549006979` at commit `f0f6c0d22ba564d7ed0ad92551889c7e209d0d8e` verifies exact dependency hashes, unit tests, clean build/JAR, core server boot, gameplay server boot, containment checks and a headless client render-thread startup smoke. Remaining M0 work is the real project impact/damage/heal/status transaction, one project-authored external-creature spawn/stat/loot binding, registry sync proof and quest save/reload/rejoin proof.
