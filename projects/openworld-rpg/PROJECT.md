...
The narrow M0 source/bootstrap and dependency-runtime layer now exists: Fabric toolchain, mod entrypoint, integration dependency manifest, `core/gameplay/essential` profiles, validation primitives, integration registry, actor-overlay schema/validation, unit tests and project CI. The `gameplay` profile co-loads the current 10 foundation/safety JARs plus 3 curated creature JARs and reaches dedicated-server ready state; see `M0_GAMEPLAY_RUNTIME_BOOT_2026-09-18.md`. The first authority seam is now implemented at commit `579354c1154c8c60ad534fe3c6558a6a57d33e66`: Better Combat melee reaches a project-owned server damage decision before the one canonical primary damage application, and vanilla sweep damage is suppressed while a Better Combat attack is active. Workflow run `35325649844` proves compile/unit/build, core-profile startup, exact Better Combat runtime API binding and gameplay-profile server ready state. **Real player-to-target hit execution is still NOT TESTED**, so duplicate-damage behavior is code-reviewed but not yet runtime-hit-proven. Remaining open adapters are Spell Engine resource/cooldown/impact routing, MobFilter ecology rules and external-creature spawn/stat/loot overrides.
