...
- 10 foundation/safety JARs + 3 curated creature JARs are pinned to exact admitted versions/bytes and co-load in the `gameplay` profile. Workflow run `35555391230` verifies unit tests, clean build/JAR, core dedicated-server boot, gameplay dedicated-server boot, donor-containment startup rules, the R01 Earthloong exact registry/profile binding at `SERVER_STARTING`, and a headless gameplay-client render-thread startup smoke at Openworld code state `836657164f35f0988ad665d7119ce570d41d3f3d`.
...
- no broad player-facing R01 implementation or persistent project save format exists yet
...
The narrow M0 source/bootstrap and dependency-runtime layer now exists: Fabric toolchain, mod entrypoint, exact dependency/version manifest, `core/gameplay/essential` profiles, validation primitives, integration registry, actor-overlay schema/validation, unit tests and project CI. The actor-overlay validator requires the full canonical 13-dimension policy surface, including movement/combat AI, recipes/worldgen and capture/duplication ownership. Normal runtime defaults to `gameplay`; `core` is isolation-only. Better Combat melee reaches a project-owned server damage decision before the one canonical primary damage application, the project mixin is conditionally applied only when Better Combat is present, and donor fallback weapon assignment/vanilla sweeping are contained. **Real player-to-target Better Combat hit execution is still NOT TESTED.** Spell Engine has a project-owned resource/cooldown transaction seam with cast-process identity so long casts cannot double-spend or become falsely rejected after the short duplicate-callback window. Canonical `Arc Bolt` still uses 12 Mana / 60 ticks; WIL changes preserve Mana percentage instead of creating a free refill; combat activity extends the out-of-combat regeneration threshold; disconnect removes transient combat state and any bound combat snapshot. Donor spell-book creation/unbinding, donor item/durability/exhaust costs, weapon fallback and RPG-Series loot injection are disabled at startup for the gameplay profiles. The canonical offensive-stat curve now explicitly uses `x = S - 5` with the documented 0.70 floor, removing the prior sub-5 formula contradiction. A project-owned pure impact transaction core implements the canonical direct-damage order, `GearScale`, offensive-stat multiplier, Defense/MR mitigation with the 70% normal armor cap and 80% routine total-mitigation cap, healing/effective-overheal resolution, strongest/refresh status merging, and poise pressure/break calculation. A new server-owned `PlayerCombatSnapshotStore` has **no fallback/default offensive snapshot**: until real progression/equipment code binds one, project spell damage fails closed rather than borrowing Spell Engine `enginePower`, vanilla attack damage or invented bootstrap stats. The Arc Bolt policy now has a canonical direct-magic impact resolver that consumes only project source/target snapshots and ignores donor power/impact-total values for damage authority. The Minecraft-side `project_direct_magic` damage path is also implemented; it applies an already-resolved final amount through a project damage type that bypasses vanilla armor/effect/enchantment/resistance/shield/cooldown/knockback layers so project mitigation is not applied twice, while deliberately not bypassing invulnerability/creative protections. The first real external-actor binding targets exact registry id `threateningly_mobs:the_earthloong`: the project validates the 13-dimension overlay, owns the authored spawn utility, applies canonical R01 **4900 HP**, exposes **DEF 45 / MR 35 / Poise 190**, converts that profile into the project target snapshot, tags capture suppression, and suppresses donor XP plus the donor entity loot table. Unit tests prove Earthloong MR/Poise feed the project resolver, the sub-5 stat floor works, missing player snapshots do not invent stats, and Arc Bolt's direct-magic resolver ignores a deliberately absurd donor power value while producing the canonical project result. Exact external registry validation runs at `SERVER_STARTING` so it remains strict without depending on Fabric ModInitializer ordering. The runtime custom-impact bridge is therefore **implemented but currently unarmed for real players** until authoritative progression/equipment code binds their combat snapshot; persistent runtime poise-state application is also still pending. A real Arc Bolt hit, real Earthloong battle and exactly-one observed runtime damage result remain **NOT TESTED**. The authored Earthloong spawn utility has not yet been invoked by an encounter controller, and the replacement project reward transaction is still pending. Workflow run `35555391230` at code state `836657164f35f0988ad665d7119ce570d41d3f3d` verifies exact dependency hashes, unit tests, clean build/JAR, core server boot, gameplay server boot including the delayed exact Earthloong registry gate, containment checks and a headless client render-thread startup smoke. Remaining M0 work is centered on binding the authoritative player snapshot from real progression/equipment state, exercising one real Arc Bolt → Earthloong damage event and binding runtime poise state, then the Earthloong encounter-controller/reward path, registry-sync proof and quest save/reload/rejoin proof.
