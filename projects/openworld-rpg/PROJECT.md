...
The narrow M0 source/bootstrap and dependency-runtime layer now exists: Fabric toolchain, mod entrypoint, integration dependency manifest, `core/gameplay/essential` profiles, validation primitives, integration registry, actor-overlay schema/validation, unit tests and project CI. The `gameplay` profile co-loads the current 10 foundation/safety JARs plus 3 curated creature JARs and reaches dedicated-server ready state; see `M0_GAMEPLAY_RUNTIME_BOOT_2026-09-18.md`. Better Combat melee reaches a project-owned server damage decision before the one canonical primary damage application, and vanilla sweep damage is suppressed while a Better Combat attack is active. **Real player-to-target Better Combat hit execution is still NOT TESTED.** Spell Engine now also has a reflection-isolated server admission gate: any future `openworld_rpg:*` spell is fail-closed unless a project authority policy is explicitly registered, while non-project spells remain outside project ownership. The 13 dependency contracts now enforce the exact distributed `fabric.mod.json` versions rather than merely recording them. Commit `2566170d51b00557dbf000fdd6c34607707902b9` / workflow run `35329917184` proves unit/build/JAR verification, exact-version gameplay validation, core/gameplay dedicated-server boot and explicit Better Combat + Spell Engine adapter activation. This **does not** yet implement the final project Mana/cooldown/impact transaction for Spell Engine. Remaining M0 work includes that real project-spell transaction, MobFilter ecology rules and one external-creature spawn/stat/loot override.
