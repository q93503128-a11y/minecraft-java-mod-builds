...
- 10 foundation/safety JARs + 3 curated creature JARs are pinned to exact admitted versions/bytes and co-load in the `gameplay` profile. Latest completed Openworld verification run `35676406598` passes unit tests, clean build/JAR, core dedicated-server boot, gameplay dedicated-server boot, donor-containment startup rules, exact Earthloong registry/runtime proof, canonical Arc Bolt and Better Combat bound-actor damage bridges, player MaxHP/VIT and Stamina/END domain tests, canonical dodge/guard/perfect-guard/guard-break authority tests including perfect-guard-only committed attacks, persistent armor/shield defensive-equipment publication tests, project-owned external-actor outgoing HP-damage containment, and gameplay-client startup smoke.
