...
The narrow M0 source/bootstrap and dependency-runtime layer now includes persistent progression, persistent equipped-loadout authority, and the first **actual Spell Engine spell resource** for `openworld_rpg:arc_bolt`. The spell resource binds the locked project values (22-block range, project CUSTOM impact handler, donor-neutral cost) and uses an initial 1.0 s cast / 1.0 blocks-per-tick projectile timing copied from the current Wizards 26.2 Arcane Bolt reference already named in `CLASS_COMBAT_KITS.md`; those two timing/presentation values remain playtest-tunable while damage, Mana, cooldown and Poise stay project-owned. The resource is not globally assigned to a normal item yet, so it does not leak a bootstrap weapon into ordinary gameplay. `M0PlayerVerificationBootstrap` is disabled by default and, only when the JVM flag `openworld_rpg.m0PlayerVerification=true` is supplied, prepares a joining player as a legal Lv8 Mage with +7 INT and ItemLv8 Staff project loadout in a disposable test world. `PlayerCombatBuildPublisher` now also synchronizes effective WIL into the server-owned Mana state when a live build is published. The existing gameplay runtime proof still resolves exact Earthloong to WeaponPower 30 / WeightedStat 10.95 / 29 damage / 4.25 poise, canonical HP 4900→4871 and Poise 190→185.75. Workflow run `35563533539` at code state `8c74f8ca37b1a63a3f6bb280bb23943f8ba90136` passes the Arc Bolt resource contract test, unit tests, clean build/JAR, core server, gameplay server runtime proof and client startup smoke. A **real joined-player Arc Bolt hit remains NOT TESTED** until a person launches the gameplay client with the verification flag, gives themselves a Spell Engine host item containing `openworld_rpg:arc_bolt`, and lands the projectile on Earthloong. Better Combat melee project authority also remains pending; source audit confirmed its current 26.2 server path selects `PlayerAttackHelper.getCurrentAttack`, then applies the authored attack `damageMultiplier()` plus dual-wield/sweeping modifiers before vanilla `player.attack`, so project melee must replace that proposal using the project's own family cadence/cycle budget rather than reusing vanilla attack damage.
