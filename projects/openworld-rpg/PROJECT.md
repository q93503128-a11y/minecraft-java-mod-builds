...
- 10 foundation/safety JARs + 3 curated creature JARs are pinned to exact admitted versions/bytes and co-load in the `gameplay` profile. Workflow run `35558638544` verifies unit tests, clean build/JAR, core dedicated-server boot, gameplay dedicated-server boot, donor-containment startup rules, the R01 Earthloong exact registry/profile binding at `SERVER_STARTING`, the canonical player-build snapshot producer, persistent combat-Lv/per-root-class Attribute allocation state, canonical-HP/poise runtime verification harness, and a headless gameplay-client render-thread startup smoke at Openworld code state `eaed3b5fc2fd2ffddb4c5131f9cb0a91877d440f`.
...
The narrow M0 source/bootstrap and dependency-runtime layer now includes a canonical player combat-build domain and the first persistent progression slice. `PlayerProgressionState` is a server-owned persistent Fabric Data Attachment with copy-on-death semantics for global combat Lv, optional active root class and an independent Attribute allocation for Warrior/Hunter/Cleric/Mage/Guardian. Each allocation is validated against the canonical `Lv - 1` earned-point pool and pre-equipment 60-stat cap, survives Codec round-trip, and lowering Lv below an already-spent allocation fails closed. `PlayerProgressionService` owns read/mutation access; no class is auto-selected at initialization. `PlayerCombatBuildState` then combines combat Lv, active class, legal allocation and validated `EquipmentCombatState` to produce normalized WeaponPower, weighted offensive stat, applicable Physical/Magic + family power, and weapon-family poise output without reading donor spell power or vanilla attack damage. Arc Bolt consumes this build domain and requires an active Mage. The gameplay CI harness uses a canon-valid Lv8 Mage (`+7 INT`, ItemLv8 Staff), producing `WeaponPower 30 / WeightedStat 10.95 / PoiseOutput 0.85`; the exact Earthloong runtime result remains canonical HP **4900 → 4871**, proxy HP **1024 → 1017.9396**, damage **29**, Poise **190 → 185.75**, break damage-taken multiplier **1.15**. Earthloong canonical HP virtualization, non-project-damage fail-closed containment and runtime boss poise remain verified. The important remaining boundary is narrower now: the **real project equipment inventory/affix publisher does not yet exist**, so join/class/stat/equipment changes do not yet automatically publish a live `PlayerCombatBuildState` into `CombatStateServices.combatBuilds()`. Therefore a real joined-player Arc Bolt and real Better Combat melee hit remain **NOT TESTED**, despite persistent combat progression now existing. Workflow run `35558638544` at code state `eaed3b5fc2fd2ffddb4c5131f9cb0a91877d440f` passes exact dependency verification, unit tests, clean build/JAR, core server, gameplay server runtime proof and gameplay client startup smoke. Remaining M0 work is the authoritative equipment publisher + live build refresh on join/class/stat/equipment change, real player Spell Engine/Better Combat execution proof, then Earthloong encounter/reward, registry-sync and quest save/reload/rejoin proofs.
