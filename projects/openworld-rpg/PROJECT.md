...
- 10 foundation/safety JARs + 3 curated creature JARs are pinned to exact admitted versions/bytes and co-load in the `gameplay` profile. Latest completed Openworld verification run `35858374327` at code state `66f5f76b879a2b183ed1bbb6c8482520acb07a78` passes unit tests, clean build/JAR, core dedicated-server boot, gameplay dedicated-server boot, gameplay-client startup smoke, exact Earthloong registry/runtime proof, canonical Arc Bolt and Better Combat authority bridges, authoritative Bow draw-power damage binding, player vitals/active-defense tests, persistent equipment/R01/Gold/Lv-EXP/Class-Rank/Recovery-Belt state codecs, reconnect-safe one-time reward transaction tests, canonical R01 opening-loadout projection, and first-root-class starter-equipment rules.
...
- broad player-facing R01 world/UI implementation is still incomplete, but persistent server-owned save state now exists for combat Lv/EXP, per-root Class Rank/XP, Attributes, equipped combat loadout, personal R01 progression, Gold, four-slot Recovery Belt state, and reconnect-safe pending/completed reward transactions
