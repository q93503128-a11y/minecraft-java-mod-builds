...
The narrow M0 source/bootstrap and dependency-runtime layer now exists: Fabric toolchain, mod entrypoint, exact dependency/version manifest, `core/gameplay/essential` profiles, validation primitives, integration registry, actor-overlay schema/validation, unit tests and project CI. The actor-overlay validator requires the full canonical 13-dimension policy surface, including movement/combat AI, recipes/worldgen and capture/duplication ownership. Normal runtime defaults to `gameplay`; `core` is isolation-only. Better Combat melee reaches a project-owned server damage decision before the one canonical primary damage application, the project mixin is conditionally applied only when Better Combat is present, and donor fallback weapon assignment/vanilla sweeping are contained. **Real player-to-target Better Combat hit execution is still NOT TESTED.** Spell Engine has a project-owned resource/cooldown transaction seam with cast-process identity so long casts cannot double-spend or become falsely rejected after the short duplicate-callback window. Canonical `Arc Bolt` still uses 12 Mana / 60 ticks; WIL changes preserve Mana percentage instead of creating a free refill; combat activity extends the out-of-combat regeneration threshold; disconnect removes transient combat state. Donor spell-book creation/unbinding, donor item/durability/exhaust costs, weapon fallback and RPG-Series loot injection are disabled at startup for the gameplay profiles. A project-owned pure impact transaction core now implements the canonical direct-damage order, `GearScale`, offensive-stat multiplier, Defense/MR mitigation with the 70% normal armor cap and 80% routine total-mitigation cap, healing/effective-overheal resolution, strongest/refresh status merging, and poise pressure/break calculation. The first real external-actor binding targets exact registry id `threateningly_mobs:the_earthloong`: the project validates the 13-dimension overlay, owns the authored spawn utility, applies canonical R01 **4900 HP**, exposes **DEF 45 / MR 35 / Poise 190**, converts that profile into the project target snapshot, tags capture suppression, and suppresses donor XP plus the donor entity loot table. Unit tests now prove Earthloong MR and Poise are consumed by the project resolver core. Exact external registry validation runs at `SERVER_STARTING` so it remains strict without depending on Fabric ModInitializer ordering. The Spell Engine `CUSTOM` runtime impact route itself intentionally remains fail-closed because authoritative player equipment/offensive-stat snapshots and Minecraft-side damage/status/poise application are not yet bound; therefore a real Arc Bolt hit and a real Earthloong battle are still **NOT TESTED**. The authored Earthloong spawn utility has not yet been invoked by an encounter controller, and the replacement project reward transaction is still pending. Workflow run `35553404729` verifies exact dependency hashes, unit tests, clean build/JAR, core server boot, gameplay server boot including the delayed exact Earthloong registry gate, containment checks and a headless client render-thread startup smoke. Remaining M0 work is centered on binding authoritative player combat snapshots plus the Spell Engine runtime impact application, then the Earthloong encounter-controller/reward path, registry sync proof and quest save/reload/rejoin proof.
