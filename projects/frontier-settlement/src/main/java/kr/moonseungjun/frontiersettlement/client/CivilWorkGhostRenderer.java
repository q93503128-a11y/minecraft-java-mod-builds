...
public final class CivilWorkGhostRenderer {
    private static final ContextKey<GhostState> DATA_KEY = new ContextKey<>(
            Identifier.fromNamespaceAndPath(FrontierSettlement.MOD_ID, "civil_work_ghost"));
    private static final int VALID_COLOR = 0xD04ED879;
    private static final int INVALID_COLOR = 0xD0E6A04B;

    private CivilWorkGhostRenderer() {}

    public static void extract(ExtractLevelRenderStateEvent event) {
        if (!CivilWorkPlacementClient.active() || CivilWorkPlacementClient.first() == null) return;
        List<BlockPos> blocks = CivilWorkPlacementClient.ghostBlocks();
        if (blocks.isEmpty()) return;
        event.getRenderState().setRenderData(DATA_KEY,
                new GhostState(blocks, CivilWorkPlacementClient.ghostValid()));
    }
