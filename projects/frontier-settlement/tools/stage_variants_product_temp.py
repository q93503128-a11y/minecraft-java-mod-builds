...
if {e['id'] for e in lock['entries']} & {'variants_and_ventures', 'resourceful_lib', 'yacl'}:
    raise SystemExit('promotion entries already present')

promoted = [
    {'id':'variants_and_ventures','name':'Variants & Ventures','side':'client_server','source':'modrinth','project_id':'lNDRiXkY','version_id':'e7ZWTYfe','version':'1.0.26+mc26.2','loader':'neoforge','required':True,'license':'CC-BY-NC-ND-4.0','integration_role':'dependency','dependencies':['resourceful_lib','yacl'],'note':'Official binary dependency only; Frontier does not copy or modify its code/assets.'},
    {'id':'resourceful_lib','name':'Resourceful Lib','side':'client_server_library','source':'modrinth','project_id':'G1hIVOrD','version_id':'hu7wvfqr','version':'5.0.0','loader':'neoforge','required':True,'license':'MIT','integration_role':'dependency_library','dependencies':[],'note':'Required by Variants & Ventures; first official 26.2 NeoForge release selected for 26.2.0.38-beta compatibility testing.'},
    {'id':'yacl','name':'YetAnotherConfigLib (YACL)','side':'client_server_library','source':'modrinth','project_id':'1eAoo2KR','version_id':'I01Wcg6G','version':'3.9.5+26.2-neoforge','loader':'neoforge','required':True,'license':'LGPL-3.0-or-later','integration_role':'dependency_library','dependencies':[],'note':'Required by Variants & Ventures; official 26.2 NeoForge build.'}
]
...
status={
 'schema':1,'status':'baseline_dedicated_server_pre_eula_pass',
 'frontier':{'version':'0.1.0-alpha.73','jar_sha256':'d300d11a8b6fe595f98c574ef9c4f71a86db6bebaaf74522a97b0bffcc7ef43c'},
 'minecraft':'26.2','neoforge':'26.2.0.38-beta',
 'baseline':{'required_client_entries':13,'required_server_entries':12,'server_payload_jars_including_frontier':13,'preflight_run_id':32822428896,'conclusion':'success'},
 'scope':{'eula_accepted':False,'fresh_world_generated':False,'client_launched':False,'spawn_density_validated':False},
 'expanded_candidate':{'variants_and_ventures_promoted':True,'required_client_entries_expected':16,'required_server_entries_expected':15,'server_payload_jars_including_frontier_expected':16,'binary_resolution_verified':False,'dedicated_server_preflight_verified':False}
}
...
old="    'lootr', 'sophisticated_backpacks', 'sophisticated_core', 'jade', 'xaeros_minimap'\n"
new="    'lootr', 'sophisticated_backpacks', 'sophisticated_core', 'jade',\n    'variants_and_ventures', 'resourceful_lib', 'yacl', 'xaeros_minimap'\n"
if old not in test: raise SystemExit('test expected-set anchor missing')
test=test.replace(old,new,1).replace('len(required) != 13','len(required) != 16',1)
...
if anchor not in test: raise SystemExit('test server-lock anchor missing')
...
if old_intro not in readme: raise SystemExit('README intro anchor missing')
...
if old_counts not in readme: raise SystemExit('README count anchor missing')
...
- Alex's Mobs Continued — large content gain, but needs a separate stability and spawn-balance pass.
"""
if old_deferred not in readme: raise SystemExit('README deferred anchor missing')
readme=readme.replace(old_deferred,new_deferred,1)
readme_path.write_text(readme,encoding='utf-8')
print('Variants & Ventures product candidate staging: PASS')
