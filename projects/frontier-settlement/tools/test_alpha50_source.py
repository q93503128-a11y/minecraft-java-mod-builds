...
# unchanged. Only exact assertions for intentionally superseded Alpha.49 civil wording are adapted;
# all other inherited runtime/source contracts execute against the current tree as written.
...
must(state, (
    'public record CivilWorkState(boolean active,', 'PHASE_CUT = 0', 'PHASE_FILL = 1', 'PHASE_RETURN = 2',
    'int earthBank', 'int completedSteps', 'int initialCutBlocks', 'int initialFillBlocks',
    'public static final Codec<CivilWorkState> CODEC', 'earthBank + 1',
    'Math.max(0, earthBank - 1)', 'beginReturn()',
...
must(service, (
    ...
    'data.replace(project.afterCut())', 'boolean importedFill = project.earthBank() <= 0',
    ...
    'data.replace(project.afterFill())', 'data.replace(project.beginReturn())',
...
forbid(service, ('forceChunk', 'setChunkForced', 'getChunk(', 'teleportTo(', 'destroyBlock(', 'dropResources(',
                 'new Villager(', 'EntityTypes.VILLAGER'), 'alpha.50 civil authority safety')
...
must(supply, (
    ...
    'SettlementStorageService.storagePositions(data)', 'stack.is(Items.DIRT) || stack.is(Items.COARSE_DIRT)',
    ...
    'SettlementStorageService.findExtractionTarget(level, data', 'builder.getNavigation().moveTo(',
...
must(network, (
    'private static final String PROTOCOL = "7"', 'CivilWorkPreviewPayload.TYPE', 'CivilWorkRequestPayload.TYPE',
    'SettlementNetwork::handleCivilWorkRequest', 'SettlementCivilWorkService.check(player,first,second)',
...
must(client, ('first = target', 'first.getY()', 'CivilWorkRequestPayload',
              'if (maxX - minX + 1 > 13 || maxZ - minZ + 1 > 13) return List.of()', 'resetStart()'),
     'alpha.50 client selection')
must(build_client, (
    'else if (CivilWorkPlacementClient.active()) CivilWorkPlacementClient.resetStart()',
    'else if (CivilWorkPlacementClient.active()) CivilWorkPlacementClient.confirm()', 'CivilWorkPlacementClient.cancel()',
...
must(commands, (
    '선택영역 토목이 끝난 뒤 건물을 시작해 주세요.', '선택영역 토목이 끝난 뒤 도로를 시작해 주세요.',
    '선택영역 토목이 끝난 뒤 전초기지를 시작해 주세요.', 'SettlementCivilWorkService.phaseLabel(server)',
    '최대 13×13 / 절토·성토 ±5', '현장 earthBank 우선',
    '공동 창고 실제 흙/거친 흙 물리 운반', '가상 토사 0',
...
must(props, ('mod_version=0.1.0-alpha.50',
             'bounded selected-area civil works with physical imported fill hauling'), 'alpha.50 properties')
must(lock, ('"frontier_settlement": "0.1.0-alpha.50"', '13x13 / plus-or-minus-5 envelope',
