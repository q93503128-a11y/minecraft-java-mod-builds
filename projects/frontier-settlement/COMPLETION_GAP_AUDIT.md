...
| 외부 모드를 콘텐츠 생산 수단으로 사용 | 완료/부분 | 후보 스택/라이선스 레지스터/일부 실제 연동. 풀런타임 미검증 |
...
| 외부 모드 재료 범주/태그 수용 | 완료/부분 | additive Frontier + `c:` 태그. 풀 companion 실플레이 미검증 |
| 대량 저장 | 완료/부분 | 창고 + 수레 정거장 화물 배럴. 장기 부하 검증 필요 |
| 전체 상자 과잉 스캔 금지 | 완료/부분 | 정해진 settlement storage만 갱신 |
| 멀티 창고/HUD 장시간 정합 | 부분 | 실제 2인 장시간 검증 필요 |
| 병영 충원도 실제 자원 사용 | 완료 | Alpha.37 식량 8 + 금속 2 원자적 소비 |
...
| 높은/큰 건물 물리 시공 | 완료/부분 | 감시탑·병영도 기존 persisted scaffold/haul 사용. 실동선 검증 필요 |
| 플레이어 건축물/컨테이너 보호 | 완료/부분 | 주요 공사경로 보호. 폭발/피스톤 전면 검증은 남음 |
...
| 감시/장거리 대응 | 완료/부분 | 감시탑별 response golem, 40블록 loaded 위협 대응. 경보/실전 검증 남음 |
| **정식 주둔 병력** | **완료/부분** | Alpha.37 병영별 3슬롯, 식량/금속 충원, 영구 귀속, loaded patrol. 사람형 병사/장비/병과는 미완 |
...
- 병영 자체는 민간 `housingCapacity`를 늘리지 않는다.
- 군사 수용량은 병영당 3으로 별도 계산한다.
- 병사는 민간 `population`에 들어가지 않아 티어 승급을 밀어주지 않는다.
- 한 번에 누락 슬롯 하나만 bounded cadence로 충원한다.
- 충원 비용은 완전히 로드된 공동 저장소에서 식량 8 + 금속 2.
- barracks 좌표 + slot tag로 영구 귀속한다.
- 언로드된 병영 주변은 사망으로 오인하지 않는다.
- 병영 병사 Iron Golem proxy의 죽음 드롭을 제거해 철 농장화를 막는다.
- 이전 `SettlementTierInfrastructureService`의 공짜 고티어 증원은 제거했고 해당 서비스는 public works만 담당한다.
- 현재 Iron Golem은 최종 병사 외형이 아니므로 병사 프레젠테이션은 `부분` 상태다.
...
| 개척 도시 | 병영·고급 제작·여러 전초 | **완료/부분** — 여러 전초 + 감시탑 + Alpha.37 병영 구현. 고급 제작 미구현 |
| 영지 | 전문 거점·후반 방어·고급 교역 | 부분 — 도로망/공공사업/감시망/주둔 병력 일부. 전문 거점·고급 교역 미완 |

## 6. 건물/인프라 계열

원본 목표: 약 `15~20개 계열`.
현재 functional `BuildingType`: **13개** + 도로 + 전초기지.
...
| 도로 | 완료 |
| 작은 다리 | 완료/부분 — 최대 6칸 수로 3폭 석재 데크. 대형/협곡 교량 미완 |
| 전초기지 중심 | 완료 |
| 수레 정거장 | 완료/부분 — 4배럴 물리 허브, 운송 적재 16→32. 실제 이동 wagon 없음 |
| 경비초소 | 완료 |
| 감시탑 | 완료/부분 — 7×7/높이 14, 탑별 loaded response guard. 경보 UI/실전 검증 남음 |
| **병영** | **완료/부분** — 15×11, 군사 3슬롯, 실물 보급 기반 충원/순찰. 최종 사람형 병사/병과/장비 미완 |
| 시장 | 완료/부분 — 유물→실물 에메랄드, 구매/고급 교역 미완 |
| 고급 제작소 | 미구현 |

원본 목표 숫자 자체를 채우기 위한 의미 없는 건물 추가는 금지한다. 남은 건설소/고급 제작소와 지역 전문화가 실제 루프를 확장해야 한다.
...
| 위험 지역 군사 거점 | 미구현 | 병영은 본진 군사 계층이며 전초 군사 전문화는 별도 미구현 |
| 연결 전 독립 재고 | 완료 | outpost stockpile |
| 연결 후 실제 운송 | 완료/부분 | 도로 운송 주민 + 본진 화물 정거장. 이동 wagon 표현 없음 |
...
| 대형 교량/터널/옹벽 | 미구현/후순위 | 원본 작은 토목 범위를 넘는 고급 토목 |
...
- **Alpha.36**: 감시탑 + tower-assigned loaded response guard, 40블록 대응, creeper 강제추격 제외.
- **Alpha.37**: 병영 + 별도 군사 3슬롯 + 식량8/금속2 충원 + persistent slot assignment + drop exploit 방지 + free tier garrison 제거.
...
| Weapons Expanded | 부분 강화 — 외부 무기 인식 + 작업장 정비. 병영 병사 장비 연동은 미완 |
| Lootr | 외부/후보검증 — 멀티 던전 보상 풀스택 확인 필요 |
| Sophisticated Backpacks | 외부/후보검증 — 탐험 왕복 피로 완화, 풀스택 확인 필요 |
| Jade | 미구현 연동 — Frontier provider 아직 없음 |
| Xaero | 미구현/후보 — waypoint/API 연동 아직 없음 |

## 11. UI / 정보

| 요구사항 | 상태 | 현재 |
| --- | --- | --- |
| Against the Storm식 compact 자원/상태 위계 | 부분 | HUD compact, 세부 상태는 command/debug 의존 |
| Manor Lords식 world-space 건설/도로 배치 | 완료/부분 | ghost placement/도로·전초 계획 구현, 실사용성 검증 필요 |
| MineColonies식 건설 재료/진행 가독성 | 부분 | 실제 builder/haul/phase 있음, 전용 세부 패널 부족 |
| 방어 건물 UI 과잉 금지 | 완료 | B 팔레트 한 섹션에 경비초소→감시탑→병영, 새 키 없음 |
| 병력 상태 | 완료/부분 | `/frontier status`에 loaded garrison/capacity/cost. 일반 HUD/알림은 미완 |
| compact side notification | 미구현 | 없음 |
| Jade 건물/주민 상태 | 미구현 | provider 없음 |

## 12. 여전히 닫혀야 하는 큰 원본 갭

- 건설소;
- 고급 제작소/희귀 재료 제작;
- 시장 구매/고급 교역;
- 사람형 정식 병사 프레젠테이션·병과·장비(병영 경제/수용량 기반은 구현됨);
- 강/해안 어업·교역 전초;
- 위험 지역 군사 전초;
- 언로드 저빈도 논리 시뮬레이션;
- Terralith/구조물 발견을 실제 영지 progression에 연결;
...
1. Alpha.37 최종 source audit / Java25 clean build / JAR verify를 최종 source/docs SHA로 고정.
2. 건설소와 고급 제작소를 추가하되 직접 조작을 늘리지 않기.
3. 고급 제작에 외부 희귀 재료/Weapons Expanded 등 검증된 콘텐츠를 연결하기.
4. 강/해안 교역·어업 및 위험 지역 군사 전초 전문화.
5. full `COMPANION_LOCK` fresh-world runtime test.
6. 언로드 coarse simulation과 UI 남은 축을 닫기.
7. 실제 플레이에서 병영 3슬롯 충원/사망/재충원, 감시탑과 병영 역할 중복, 도로·전초·시장·작업장까지 전체 루프를 회귀검사.
8. 이후 사람형 병사 프레젠테이션이 실제 가치가 있을 때 external combat/weapon stack과 함께 개선.
