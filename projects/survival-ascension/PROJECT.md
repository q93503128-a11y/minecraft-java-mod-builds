...
- 웨이브는 단순 HP 배율 대신 좀비·스켈레톤 → 허스크·스트레이·마녀 → 위더 스켈레톤·일리저 → 라베저·변명자·약탈자·마녀 혼합으로 역할과 행동 패턴을 늘린다.
- 소환 후 별도 엔티티를 남기는 Evoker는 시련 직접 구성에서 제외해 실패/완료 뒤 추적되지 않은 Vex가 잔존하는 문제를 예방한다.
...
- 시련 자체는 서버 재시작 후 복구하지 않는 런타임 encounter다. 이미 지불한 비용/cooldown은 유지하며, 재시작 전 저장된 시련 태그 몹은 `EntityJoinLevelEvent`에서 활성 시련 소속 여부를 다시 검사해 고아 몹으로 남지 못하게 한다.
- 같은 JVM에서 싱글플레이 월드/서버 인스턴스를 다시 열어도 이전 서버를 참조하는 정적 시련 상태를 즉시 제거해 새 월드의 시작을 막거나 오래된 ServerLevel 참조를 유지하지 않는다.
...
- 승천 시련은 실제 소모 재료·중복 거리·플레이어별 단일 활성·cooldown·재시작 고아몹 정리·서버 인스턴스 정리·소환 잔존 방지로 무상 반복 소환과 잔존 엔티티 악용을 차단한다.
