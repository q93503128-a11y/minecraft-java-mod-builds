...
need(main, ['VERSION = "0.38.0-alpha.1"',
...
# 0.38 defendable physical outpost contract.
...
ordered(siege, ["if (!spawnWave(siege))", "if (!production.consumeSupplyCharge(player))", "ACTIVE.put(player.getUUID(), siege)"], "0.38 spawn-before-charge")

production = read("src/main/java/kr/moonseungjun/survivalascension/production/ProductionService.java")
need(production, ['ACTION_OUTPOST_SIEGE = "outpost_siege"',
    "if (ACTION_OUTPOST_SIEGE.equals(action)) { OutpostSiegeSystem.startOrStatus(player); return; }",
    "if (OutpostSiegeSystem.isActive(player))", "OutpostSiegeSystem.sendStatus(player)", "방어1"], "0.38 production routing")
infra = read("src/main/java/kr/moonseungjun/survivalascension/infrastructure/InfrastructureService.java")
need(infra, ["ProductionService.ACTION_OUTPOST_SIEGE.equals(action)", "if (OutpostSiegeSystem.isActive(player))", "전초 방어전",
    "InfrastructureSiteService.validateForFinalFunding(player, project)", "FieldDepotService.countMaterial(player, item)", "FieldDepotService.consume(player, item, amount)"], "0.38 infrastructure gate")
...
need(radial, ["전초 방어전", "Action.SIEGE", "case SIEGE -> ProductionService.ACTION_OUTPOST_SIEGE;", "앵커6블록 돌파압력"], "0.38 radial")
...
need(field, ["toggleWarehouseNearest", "activeStorageBarrelCount", "FieldDepotData.MAX_LINK_RADIUS", "FieldDepotData.MAX_LINKED_BARRELS_PER_DEPOT",
    ...
    "offloadBulkMaterials", "ItemStack.isSameItemSameComponents(source, existing)", "container.canPlaceItem(slot, source)", "source.copyWithCount(move)"], "0.37/0.35 physical logistics")
