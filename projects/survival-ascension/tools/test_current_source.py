...
need(mutation,['MUTATION_KEY = "survivalascension_endgame_mutation"','MUTATION_CHANCE = 0.18D','WorldAscensionData.get(level.getServer()).stage() < 2','contains("SPAWNER")','mob instanceof AbstractSkeleton','mob instanceof Zombie','Mutation.WITHERED','Mutation.PHASE','Mutation.PLAGUE','MobEffects.WITHER, 80, 0','MobEffects.POISON, 120, 0','level.getRandom().nextFloat() >= 0.55F','now + 45L','player.giveExperiencePoints(10)','nextFloat() < 0.35F','Items.ECHO_SHARD'],"endgame mutations regression")
...
need(trial,["Gateways to Eternity","ECHO_SHARD_COST = 32","AMETHYST_COST = 64","DRAGON_BREATH_COST = 8","TOTAL_WAVES = 4","WAVE_TIMEOUT_TICKS = 1200","START_COOLDOWN_TICKS = 2400","EXCLUSION_RADIUS = 96.0D","AscensionTrialDoctrine.random","maybeReinforce","reinforcementTypeId","reinforcementsTriggered","initialWaveCount","InfrastructureProject.ASCENSION_NEXUS","WorldAscensionData.get(server).stage() < 2","EntitySpawnReason.TRIGGERED","ServerBossEvent","BuiltInRegistries.ENTITY_TYPE.containsKey(identifier)","BuiltInRegistries.ENTITY_TYPE.getValue(identifier)",'"minecraft:ravager"','"minecraft:pillager"','"minecraft:wither_skeleton"','"minecraft:enderman"','"minecraft:witch"',"trial.doctrine == AscensionTrialDoctrine.PURSUIT","mob.getNavigation().moveTo(owner, 1.35D)","AscensionAffixes.createEliteDrop(trial.level.getRandom(), 3)","Items.NETHERITE_SCRAP, 2","Items.DIAMOND, 4","onEntityJoin(EntityJoinLevelEvent event)","TRIAL_OWNER_KEY","active.mobIds.contains(mob.getUUID())","event.setCanceled(true)","removeStaleServerTrials(server)","trial.level.getServer() != server","distanceToCenterSqr","입장 재료는 반환되지 않습니다"],"Ascension Trial doctrine regression")
...
need(third,["Hostiles Are Too Easy","CC0 1.0 Universal","0.19","Warband","Veinminer++","Gateways to Eternity","Copyright (c) 2020 Brennan Ward","0.22 additionally adapts at a high level the idea of wave-changing modifiers"],"third-party notices")

affix=(ROOT/"src/main/java/kr/moonseungjun/survivalascension/equipment/AscensionAffixes.java").read_text(encoding="utf-8")
need(affix,["AFFIX_POOL",'AWAKENED = "awakened"',"public static boolean canAwaken","currentAffixes(stack).size() == 3","if (!canAwaken(stack)) return false","missing.size() != 2","public static boolean awaken","public static boolean isAwakened","awakened ? 4 : rarity","chosen.add(missing.get","§5[각성 신화]","adjustMiningArea","adjustWoodcuttingLimit","adjustHarvestArea","if (base <= 1","if (base <= 0.0D"],"awakened affix regression")
...
need(reforge,["ACTION_REFORGE","ACTION_SALVAGE","ACTION_AWAKEN","awakeningCosts","AscensionAffixes.canAwaken(held)","재료는 소비하지 않았습니다","AscensionAffixes.awaken","Items.AMETHYST_SHARD, 256","Items.DIAMOND, 24","Items.NETHERITE_SCRAP, 8","Items.ECHO_SHARD, 64","Items.DRAGON_BREATH, 16","Items.AMETHYST_SHARD, 128","Items.NETHERITE_SCRAP, 4","Items.ECHO_SHARD, 16"],"Mythic awakening economy")
