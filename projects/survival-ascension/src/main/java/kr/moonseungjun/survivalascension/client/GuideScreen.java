...
public final class GuideScreen extends Screen {
    ...
    private static List<Line> overviewLines() {
        return List.of(
                ...
                h("채굴"), p("광역 굴착 + 연결 광맥. M→채굴에서 자동/굴착/광맥/추출 모드를 직접 선택."),
    ...
    private static List<Line> unlockLines() {
        return List.of(
                h("채굴"), p("Lv.10 3×3+굴착모드 · Lv.30 5×5+광맥24 · Lv.60 7×7+광맥64 · Lv.90 9×9+광맥128+추출모드"),
                p("추출: 12블록 반경의 이미 로드된 구역에서 같은 광석군을 비연결 상태까지 찾아 정상 채굴 처리."),
    ...
    private static List<Line> controlLines() {
        return List.of(
                h("M · 통합 메뉴"), p("숙련 / 채굴 / 건축 / 장비 / 가이드 / 해금표 / 통계 / 조작."),
                h("M → 채굴"), p("자동: 광석은 광맥, 일반은 굴착. 굴착: 평면 광역. 광맥: 연결 광석만. 추출: Lv.90 비연결 광석 탐색."),
