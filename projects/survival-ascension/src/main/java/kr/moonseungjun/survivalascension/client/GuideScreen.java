...
public final class GuideScreen extends Screen {
    ...
    private static List<Line> overviewLines() {
        return List.of(
                ...
                h("대원정 현장 지령"), p("지역은 들어가기만 하면 발견되지만, 완수하려면 플레이어별로 배정된 현장 지령의 모든 task를 그 지역 안에서 직접 수행해야 합니다."),
                h("지령 변주"), p("새 지역은 표준형 또는 혼합형 지령 중 하나가 배정됩니다. 혼합형은 벌목+횡단, 건축+횡단, 수확+전투, 이동+돌진, 채굴+전투처럼 실제 플레이 방식을 바꿉니다."),
    ...
    private static List<Line> unlockLines() {
        return List.of(
                ...
                h("혼합 지령 예"), p("삼림 벌목64+횡단240 · 고산 횡단360+돌진12 · 심층 채굴128+적10 · 네더 적16+채굴96 · 엔드 적20+횡단360"),
    ...
    private static List<Line> controlLines() {
        return List.of(
                ...
                h("원정 판정"), p("실제 대량 벌목·대량 건축·성숙 작물·정상 이동·항해·곡괭이 채굴·적대몹 처치·성공한 돌진만 해당 지령 task에 기록."),
                h("이동 원정"), p("도보 횡단은 텔레포트·비행·수영·탑승 제외. 대양 항해는 수영/수중/탑승 이동만 인정합니다."),
                h("0.24 세이브"), p("기존 발견 지역은 표준 지령으로 승계하고 진행량을 보존합니다. 이미 완수한 지역과 현장 숙련도 유지됩니다."),
                h("R · 기동"), p("지상/공중 돌진. 성공한 R 사용은 돌진 task가 배정된 고산/빙설 지령에서만 기록됩니다."),
