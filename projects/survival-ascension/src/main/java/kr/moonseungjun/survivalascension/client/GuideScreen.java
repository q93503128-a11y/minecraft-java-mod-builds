...
public final class GuideScreen extends Screen {
    ...
    private static List<Line> overviewLines() {
        return List.of(
                ...
                h("통합 물류 백본 / 현장 일괄 적재"), p("가까운 등록 배럴/활성 전초는 산업·인프라·재련 재고를 공급합니다. 0.35부터 주 인벤토리의 대량 자원도 한 번에 가까운 사용 가능 배럴부터 적재할 수 있으며 핫바/장비는 유지됩니다."),
                h("물리 준공 현장"), p("0.36부터 산업 가공소·정점 추적소·승천 중추는 마지막 자원 투입 전에 실제 배럴 중심의 건축 현장을 서버가 확인합니다. 이미 완공된 기존 월드는 그대로 인정됩니다."),
    ...
    private static List<Line> unlockLines() {
        return List.of(
                ...
                h("산업 / 물류"), p("Stage1 산업 가공소 → 4계통 생산 → 보급권 → 배럴 거점 → 물리 전초기지. 등록 배럴은 일반32블록, 활성 전초는64블록 안에서 산업/인프라/재련 투입 및 0.35 일괄 적재에 사용됩니다."),
                h("산업 가공소 준공"), p("마지막 투입 시 4블록 내 실제 배럴 필요. 반경6 안 석재벽돌48 · 철블록4 · 용광로2 · 석재절단기1 · 호퍼2를 실제로 배치합니다."),
                h("정점 추적소 준공"), p("마지막 투입 시 4블록 내 자기 등록 배럴 필요. 반경6 안 석재벽돌32 · 금블록4 · 자석석1 · 지도제작대1 · 과녁4."),
                h("승천 중추 준공"), p("마지막 투입 시 4블록 내 자기 등록 배럴 필요. 반경6 안 흑요석32 · 우는흑요석8 · 신호기1 · 마법부여대1 · 엔더상자1."),
    ...
    private static List<Line> controlLines() {
        return List.of(
                ...
                h("산업 생산"), p("M → 인프라 → 산업 가공소. 생산·출고·배럴 연결·일괄 적재·전초 승격·원정 작전·현장 복귀를 같은 방사형 메뉴에서 처리합니다."),
                h("시설 최종 준공"), p("산업 가공소·정점 추적소·승천 중추의 남은 재료를 한 번에 완납할 수 있는 순간에는 먼저 현장 검사를 합니다. 현장이 부족하면 그 호출은 마지막 프로젝트 재료를 하나도 소비하지 않습니다."),
                h("준공 검사 안전"), p("배럴과 시설 블록은 서버가 반경4/6에서 직접 찾으며 이미 로딩된 블록만 봅니다. mayInteract를 통과해야 하고 청크 강제로드·클라이언트 좌표·가상 건물 판정은 없습니다."),
                h("현장 일괄 적재"), p("호퍼 아이콘을 선택하면 주 인벤토리 슬롯9~35의 광물·석재·목재·농산물·주요 인프라 자원만 가까운 사용 가능 배럴부터 채웁니다. 핫바0~8과 장비/보조손은 건드리지 않습니다."),
                ...
                h("물류 / 전초"), p("배럴 거점은 일반32블록, 활성 전초는64블록까지 실제 재고를 대량 건축/관개/산업 투입과 일괄 적재에 사용합니다. 청크 강제로드 없음."),
