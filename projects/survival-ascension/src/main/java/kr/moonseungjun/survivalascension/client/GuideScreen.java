...
public final class GuideScreen extends Screen {
    ...
    private static List<Line> overviewLines() {
        return List.of(
                ...
                h("전초 효과"), p("소유자가 전초64블록 안에 있을 때 해당 배럴 물류 반경이32→64로 늘고, 전초24블록 안 자연 적대몹 스폰이 억제됩니다. 사건/정점/시련의 강제 스폰은 유지됩니다."),
    ...
    private static List<Line> unlockLines() {
        return List.of(
                ...
                h("전초 안전 규칙"), p("소유자가 같은 차원 64블록 안에 있고 구조가 유지될 때만 활성. 청크 강제로드 없음. NATURAL 적대몹만 억제하고 TRIGGERED 전투는 보존."),
    ...
    private static List<Line> statsLines() {
        ...
        out.add(p("/ascension stats에서 원정/사건, 정점, 산업 사이클·보급권·물류 거점·전초기지 활성 상태를 함께 확인합니다."));
        return out;
    }
    private static List<Line> controlLines() {
        return List.of(
                h("M · 통합 메뉴"), p("숙련 / 채굴 / 건축 / 장비 / 인프라 / 가이드 / 닫기."),
                h("산업 생산"), p("M → 인프라 → 산업 가공소. 생산·출고·배럴 연결·전초 승격·상태 확인을 한 방사형 하위 메뉴에서 처리합니다."),
                h("물류 거점 연결"), p("연결할 배럴4블록 안에서 선택. 미등록은 보급권1로 등록, 내 거점이면 해제. 해제 시 전초 승격도 사라집니다."),
                h("전초기지 승격"), p("등록 배럴4블록 안에서 선택. 주변5블록의 침대/모닥불/작업대/화로 구조와 보급권2·철32·금8·석탄32을 서버가 확인합니다."),
                h("물류 인출"), p("대량 건축/관개는 인벤토리 우선, 부족하면 가까운 활성 배럴 실제 스택을 사용합니다. 전초 배럴은 활성 시 64블록까지 지원합니다."),
