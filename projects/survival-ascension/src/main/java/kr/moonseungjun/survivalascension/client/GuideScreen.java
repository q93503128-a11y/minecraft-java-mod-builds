...
public final class GuideScreen extends Screen {
    ...
    private static List<Line> overviewLines() {
        return List.of(
                ...
                h("외부 장비와 승천 각인"), p("콘텐츠 팩이나 다른 모드의 장비가 표준 검/스피어/메이스/활/쇠뇌/곡괭이/도끼/삽/괭이/방패 또는 머리/가슴/다리/발 방어구 태그를 사용하면 장비 메뉴의 승천 각인으로 Survival Ascension affix 체계에 편입할 수 있습니다. 각성 단계는 정예, 전설 단계는 승천, 종말 단계는 신화 등급으로 각인합니다."),
                ...
                h("방패 방어 파동"), p("표준 방패로 실제 차단에 성공하면 전투 Lv30부터 주변 적을 피해 없이 밀어내는 방어 파동이 발생합니다. Lv30=2.5블록/2체, Lv60=3.5/4, Lv90=4.5/6, Lv100=5.5/8, 현장 숙련=6.5블록/10체입니다. Shift 차단은 파동 없는 정밀 방어이며, 방패 affix 압력·파동·대응·진압·반동은 밀침·반경·재사용·대상·수직 반동만 제한적으로 확장합니다."),
                ...
                h("물류 통과 창고군"), p("산업 가공소 완공 후 마인크래프트 기본 통(Barrel)을 물류 거점으로 등록합니다. 등록 한도는 산업 단계 3개, 토목 공사소 완공 후 6개, 종말 단계 승천 중추 완공 후 9개로 늘어 9지역 전선을 장기 유지할 수 있습니다. 거점 통 하나에 반경6의 실제 통을 최대8개 연결하며 실제 인벤토리를 그대로 사용합니다."),
    ...
    private static List<Line> unlockLines() {
        return List.of(
                ...
                h("승천 각인"), p("M → 장비 → 승천 각인. affix가 없는 표준 검/스피어/메이스/활/쇠뇌/곡괭이/도끼/삽/괭이/방어구/방패 태그 장비를 현재 월드 승천 단계의 정예/승천/신화 등급으로 편입합니다. 외부 모드 장비도 같은 태그를 사용하면 별도 의존성 없이 지원됩니다."),
    ...
    private static List<Line> controlLines() {
        return List.of(
                ...
                h("승천 각인"), p("주 손의 affix 없는 검/메이스/활/쇠뇌/곡괭이/도끼/삽/괭이/방어구/방패 태그 장비를 M → 장비 → 승천 각인으로 편입합니다. 각인 재료도 가까운 사용 가능 물류 통을 먼저 쓰고 부족분만 인벤토리에서 사용합니다."),
