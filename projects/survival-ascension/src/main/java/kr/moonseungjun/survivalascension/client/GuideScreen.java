...
public final class GuideScreen extends Screen {
    ...
    private static List<Line> unlockLines() {
        return List.of(
                ...
                h("승천 각인"), p("K → 장비 → 승천 각인. 승천 옵션이 없는 표준 검/스피어/메이스/활/쇠뇌/곡괭이/도끼/삽/괭이/방어구/방패 태그 장비를 현재 월드 승천 단계의 정예/승천/신화 등급으로 편입합니다. 외부 모드 장비도 같은 태그를 사용하면 별도 의존성 없이 지원됩니다."),
    ...
    private static List<Line> controlLines() {
        return List.of(
                h("K · 통합 메뉴"), p("숙련 / 채굴 / 건축 / 장비 / 인프라 / 가이드 / 닫기."),
                ...
                h("일반 화물 적재"), p("K → 인프라 → 산업 가공소 → 물리 화물 수레. 완성된 출발 하역장 레일 위 빈 상자 광산수레에서 Shift 없이 선택하면 그 전초의 기존 대량자원을 수레 용량만큼 적재합니다."),
                ...
                h("도로/교량 시공"), p("K → 건축 → 도로/교량. 토목 공사소 완공 후 블록 하나를 놓으면 같은 높이의 3폭 바닥을 앞으로 대량 시공합니다."),
    ...
    private static String effect(SkillType skill, int level) {
        return switch (skill) {
            ...
            case MOBILITY -> String.format(Locale.ROOT, "이속 %.2f× / 단차 %.2f / 안전낙하 %.0f / V %s", SkillTuning.mobilitySpeedMultiplier(level), SkillTuning.mobilityStepHeight(level), SkillTuning.mobilitySafeFallDistance(level), level < 30 ? "잠김" : "돌진");
        };
    }
