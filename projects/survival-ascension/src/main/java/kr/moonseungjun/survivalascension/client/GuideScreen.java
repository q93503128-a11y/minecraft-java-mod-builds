...
public final class GuideScreen extends Screen {
    ...
    private static List<Line> overviewLines() {
        return List.of(
                ...
                h("토목 공사소 / 도로·교량"), p("0.41부터 전설 단계 토목 공사소를 실제 물류 배럴 기반 준공 현장과 함께 완공하면 건축 Lv.60부터 바라보는 방향으로 3폭 장거리 도로/교량 바닥을 실제 블록으로 시공합니다."),
                h("물리 요새 방어"), p("전초 반경6~12에 실제 벽 계열/철창/네더벽돌 울타리를 사분면마다 12열 이상 세우면 보급권2로 4공세 요새 방어를 열 수 있습니다. 별도 피해감소 버프 없이 실제 벽의 길막이 곧 방어력입니다."),
                h("공성 파괴자"), p("요새의 고유 4번째 공세에서 Ravager/Vindicator가 실제 방어진에 닿으면 벽 계열 블록을 부술 수 있습니다. 일반 3공세 방어전은 블록 파괴가 없고, 파괴된 자재는 드롭되어 즉시 수리할 수 있습니다."),
    ...
    private static List<Line> unlockLines() {
        return List.of(
                ...
                h("토목 공사소"), p("월드 승천1단계. 석재벽돌2048 · 조약돌1536 · 자갈1536 · 철256 · 구리256. 최종 투입은 자기 등록 배럴4블록 안의 물리 토목 현장을 먼저 검증합니다."),
                h("도로/교량 모드"), p("건축 Lv.60 + 토목 공사소. 3폭 × 17칸, Lv.90 33, Lv.100 49, 현장 숙련65. 첫 블록과 같은 블록을 바라보는 수평 방향 앞으로 시공합니다."),
                h("토목 준공 현장"), p("등록 배럴 반경6 안에 석재벽돌48 · 비계16 · 철블록4 · 석재절단기2 · 작업대1. 현장은 최종 자원 투입 직전에만 검증되며 이후 유지보수 의무는 없습니다."),
                h("요새 방어진지"), p("활성 전초 앵커 기준 수평 반경6~12. 벽 태그 블록/철창/네더벽돌 울타리 열을 NE/NW/SE/SW 각각12 이상. 같은 x/z에 여러 높이를 쌓아도 1열입니다."),
                h("요새 방어전"), p("방어진지 통과 + 보급권2. 총4공세·전체5분. 공세 사이마다 사분면 방어진지를 다시 검사하며 일반 방어와 돌파압력200 규칙은 동일합니다."),
                h("최종 공세 파괴"), p("요새 4공세의 Ravager는 약1.5초, Vindicator는 약3초 간격으로 진행 방향의 적격 방어진 한 칸을 파괴할 수 있습니다. mobGriefing/보호 이벤트가 막으면 파괴하지 않습니다."),
                h("일반 전초 방어"), p("Stage1 산업 가공소 + 활성 전초 필요. 앵커4블록 안에서 보급권1. 총3공세·전체4분. 일반 모드의 습격대는 방어진 블록을 파괴하지 않습니다.")
        );
    }
    ...
    private static List<Line> statsLines() {
        ...
        out.add(p("인프라 진행도에서 토목 공사소를 포함한 프로젝트 재료/물리 준공 현황을 확인할 수 있습니다."));
        return out;
    }
    ...
    private static List<Line> controlLines() {
        return List.of(
                ...
                h("도로/교량 시공"), p("M → 건축 → 도로/교량. 토목 공사소 완공 후 블록 하나를 놓으면 바라보는 수평 방향 앞으로 같은 높이의 3폭 바닥을 대량 시공합니다."),
                h("도로/교량 재료"), p("첫 블록과 같은 실제 BlockItem을 인벤토리 우선 → 가까운 사용 가능 물류 배럴에서 한 칸씩 소비합니다. 막힌 칸/보호된 칸/미로딩 청크는 건너뜁니다."),
                h("토목 안전"), p("기존 건축 큐의 전역64블록/틱·플레이어당8블록/틱 예산, mayInteract, NeoForge 배치 이벤트, 생존성 검사를 그대로 사용합니다. 새 자동건축 엔진이나 강제 청크 로딩은 없습니다."),
                h("Shift · 정밀"), p("채굴·벌목·농사 광역 효과를 끄고 건축은 단일 배치를 우선합니다."),
                h("공성 파괴 대응"), p("요새 4공세에서 Ravager/Vindicator가 성벽에 붙으면 실제 방어진을 파괴합니다. 드롭된 벽 자재를 주워 건축 선/벽 배치로 틈을 메우세요."),
