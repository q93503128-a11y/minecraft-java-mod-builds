...
public final class GuideScreen extends Screen {
    ...
    private static List<Line> overviewLines() {
        return List.of(
                ...
                h("공성 파괴자"), p("0.40부터 요새의 고유 4번째 공세에서 Ravager/Vindicator가 실제 방어진에 닿으면 벽 계열 블록을 부술 수 있습니다. 일반 3공세 방어전은 블록 파괴가 없고, 파괴된 자재는 드롭되어 즉시 수리할 수 있습니다."),
    ...
    private static List<Line> controlLines() {
        return List.of(
                ...
                h("공성 파괴 대응"), p("4공세에서 Ravager/Vindicator가 성벽에 붙으면 실제 방어진을 파괴합니다. 드롭된 벽 자재를 주워 건축 선/벽 배치로 틈을 메우고 앵커 진입로가 열리지 않게 관리하세요."),
                h("보호 규칙"), p("공성 파괴는 mobGriefing과 NeoForge 엔티티 파괴 이벤트, 소유자의 mayInteract를 모두 통과해야 합니다. 임의 지형·상자·전초 앵커는 파괴 대상이 아닙니다."),
