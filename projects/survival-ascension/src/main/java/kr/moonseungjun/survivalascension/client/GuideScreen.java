...
public final class GuideScreen extends Screen {
    ...
    private static List<Line> overviewLines() {
        return List.of(
                ...
                h("외부 장비와 승천 각인"), p("콘텐츠 팩이나 다른 모드의 장비가 표준 검/활/쇠뇌/곡괭이/도끼/삽/괭이/방패 또는 머리/가슴/다리/발 방어구 태그를 사용하면 장비 메뉴의 승천 각인으로 Survival Ascension affix 체계에 편입할 수 있습니다. 각성 단계는 정예, 전설 단계는 승천, 종말 단계는 신화 등급으로 각인합니다."),
    ...
    private static List<Line> unlockLines() {
        return List.of(
                ...
                h("승천 각인"), p("M → 장비 → 승천 각인. affix가 없는 표준 검/활/쇠뇌/곡괭이/도끼/삽/괭이/방어구/방패 태그 장비를 현재 월드 승천 단계의 정예/승천/신화 등급으로 편입합니다. 외부 모드 장비도 같은 태그를 사용하면 별도 의존성 없이 지원됩니다."),
    ...
    private static List<Line> controlLines() {
        return List.of(
                ...
                h("승천 각인"), p("주 손의 affix 없는 검/활/쇠뇌/곡괭이/도끼/삽/괭이/방어구/방패 태그 장비를 M → 장비 → 승천 각인으로 편입합니다. 각인 재료도 가까운 사용 가능 물류 통을 먼저 쓰고 부족분만 인벤토리에서 사용합니다."),
