...
public final class GuideScreen extends Screen {
    ...
    private static List<Line> overviewLines() {
        return List.of(
                ...
                h("도로·교량과 물리 화물"), p("전설 단계 토목 공사소와 건축 Lv.60을 갖추면 3폭 장거리 도로/교량을 실제 블록으로 시공합니다. 산업+토목 조건을 갖춘 활성 전초끼리는 레일 위 실제 상자 광산수레로 창고 자원을 운반할 수 있습니다."),
    ...
    private static List<Line> unlockLines() {
        return List.of(
                ...
                h("물리 화물"), p("산업 가공소+토목 공사소 필요. 활성 전초4블록 안에서 레일 위 빈 상자 광산수레를 적재하고, 같은 차원의 다른 활성 전초까지 실제 운반한 뒤 도착 창고군으로 하역합니다."),
    ...
    private static List<Line> statsLines() {
        ...
        out.add(p("산업 생산 현황에서 실제 물류 통/전초와 근처 상자 광산수레의 물리 화물 상태도 함께 확인합니다."));
        return out;
    }
    ...
    private static List<Line> controlLines() {
        return List.of(
                ...
                h("물리 화물 하역"), p("적재한 바로 그 수레를 같은 차원의 다른 자신의 활성 전초까지 레일로 운반한 뒤 다시 선택합니다. 도착 통 용량 부족 시 가능한 만큼만 옮기고 잔여 화물은 수레에 남습니다."),
                h("화물 안전 경계"), p("수레/재고 순간이동, 자동 운전, 가상 노선, 강제 청크 로딩은 없습니다. 실제 레일·실제 수레·실제 통 재고가 전부입니다."),
